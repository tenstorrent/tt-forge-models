...
"""Helper functions for NSFW-GEN-ANIME model loading and processing."""
...
def load_pipe(pretrained_model_name):
    """Load NSFW-GEN-ANIME pipeline with components in eval mode."""
    pipe = StableDiffusionXLPipeline.from_pretrained(
        pretrained_model_name, torch_dtype=torch.float32
    )
    pipe.to("cpu", dtype=torch.float32)
    for module in [pipe.text_encoder, pipe.unet, pipe.text_encoder_2, pipe.vae]:
        module.eval()
...
def stable_diffusion_preprocessing_xl(
    ...
    """Preprocess inputs for the SDXL UNet forward pass."""
