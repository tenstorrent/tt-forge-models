...
"""
Helper functions for Noob SDXL ControlNet Normal model loading and processing.
"""
...
def load_controlnet_normal_sdxl_pipe(controlnet_model_name, base_model_name):
    """Load ControlNet Normal SDXL pipeline.
...
def create_normal_conditioning_image(height=1024, width=1024):
    """Create a dummy normal map conditioning image.
    ...
    Returns:
        PIL.Image: A dummy normal map conditioning image
    """
    return Image.new("RGB", (width, height), color=(0, 0, 0))


def controlnet_normal_sdxl_preprocessing(
    pipe,
    prompt,
    control_image,
    device="cpu",
    ...
    """Preprocess inputs for ControlNet Normal SDXL model.

    Args:
        pipe: StableDiffusionXLControlNetPipeline
        prompt: Text prompt for generation
        control_image: Normal map conditioning image
