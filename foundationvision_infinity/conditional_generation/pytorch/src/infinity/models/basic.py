...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        ...
        # qkv: amp, bf16
        # NOTE: positional args (tt_torch's linear torch-function override indexes args[1]).
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        kv_compact = F.linear(kv_compact, self.mat_kv.weight, torch.cat((self.zero_k_bias, self.v_bias))).view(N, 2, self.num_heads, self.head_dim) # NC => N2Hc  # positional args (tt_torch linear override)
