...
class Infinity(nn.Module):
    ...
    def add_lvl_embeding_for_x_BLC(self, x_BLC, scale_schedule, need_to_pad=0):
        ...
        for scale_ind, patch_t_h_w in enumerate(scale_schedule):
            # NOTE: python int product (was np.array(...).prod() -> numpy int64),
            # so ptr stays a python int and the slicing traces statically on device.
            patch_t, patch_h, patch_w = patch_t_h_w
            scale_seq_len = int(patch_t) * int(patch_h) * int(patch_w)
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # drop cond
            # NOTE: guarded on cond_drop_rate>0 so the non-traceable random.random()
            # call is skipped at inference (cond_drop_rate is set to 0 by the loader).
            ...
            # NOTE: rewritten to plain broadcast-add (was .expand(B, 1, -1) on both
            # terms); avoids a dynamo tracing failure with expand under the TT path.
