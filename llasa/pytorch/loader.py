...
Llasa is a Llama-3.1-8B-Instruct backbone fine-tuned for text-to-speech: it is a
decoder-only causal LM whose vocabulary is extended with speech-codec tokens
(XCodec2). The model itself is a single-forward-pass LlamaForCausalLM that emits
logits over text + speech tokens; the separate XCodec2 vocoder that turns those
speech tokens into a waveform is out of scope for this loader. We therefore bring
it up exactly like a causal LM forward pass.
"""
...
class ModelLoader(ForgeModel):
    ...
    # Sample text to be turned into speech tokens
    ...
    def load_model(self, dtype_override=None):
        ...
        Returns:
            torch.nn.Module: The Llasa model instance (LlamaForCausalLM).
        """
        pretrained_model_name = self._variant_config.pretrained_model_name
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        # Pad input_ids and attention_mask to a static length
