...
def _patched_load_gguf_checkpoint(gguf_path, return_tensors=False, **kwargs):
    ...
    # gguf is installed dynamically by RequirementsManager after transformers is
    # already imported, so PACKAGE_DISTRIBUTION_MAPPING is stale and gguf.__version__
    # is undefined. Refresh the mapping so is_gguf_available() resolves correctly.
    import importlib
...
class ModelVariant(StrEnum):
    """Available mradermacher bartleby-qwen3.5-4b GGUF model variants for causal language modeling."""

    MRADERMACHER_BARTLEBY_QWEN3_5_4B_GGUF = "bartleby-qwen3.5-4b-GGUF"


class ModelLoader(ForgeModel):
    """mradermacher bartleby-qwen3.5-4b GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.MRADERMACHER_BARTLEBY_QWEN3_5_4B_GGUF: LLMModelConfig(
            pretrained_model_name="mradermacher/bartleby-qwen3.5-4b-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.MRADERMACHER_BARTLEBY_QWEN3_5_4B_GGUF

    GGUF_FILE = "bartleby-qwen3.5-4b.Q4_K_M.gguf"
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="mradermacher bartleby-qwen3.5-4b GGUF",
            variant=variant,
