...
class ModelLoader(ForgeModel):
    ...
    def _load_pipeline(self, dtype_override=None):
        ...
        # Materialize weights directly at the requested dtype. SD3-medium is
        # ~30 GB in fp32 but only ~15 GB in bf16; loading fp32 first and casting
        # afterwards OOM-kills a 31 GB host, so push the dtype into from_pretrained.
    ...
    def get_mesh_config(self, num_devices: int):
        ...
        SD3-medium's MMDiT is brought up across multiple chips with Megatron-1D
        tensor parallelism over a ``(None, "model")`` mesh. See
        ``src/shard_specs.py``.
    ...
    def load_shard_spec(self, model):
        ...
        Args:
            model: the ``SD3Transformer2DModel`` returned by ``load_model``.
