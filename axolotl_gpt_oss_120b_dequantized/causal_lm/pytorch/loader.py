...
"""
Axolotl GPT-OSS 120B Dequantized model loader implementation for causal language modeling.
"""
import torch
...
class ModelVariant(StrEnum):
    """Available Axolotl GPT-OSS 120B Dequantized model variants."""

    GPT_OSS_120B_DEQUANTIZED = "Default"


class ModelLoader(ForgeModel):
    """Axolotl GPT-OSS 120B Dequantized model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.GPT_OSS_120B_DEQUANTIZED: LLMModelConfig(
            pretrained_model_name="axolotl-ai-co/gpt-oss-120b-dequantized",
            max_length=256,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.GPT_OSS_120B_DEQUANTIZED
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="Axolotl-GPT-OSS-120B-Dequantized",
            variant=variant,
    ...
    def get_mesh_config(self, num_devices: int):
        if num_devices == 32:  # Galaxy
            ...
            raise ValueError(
                "Axolotl GPT-OSS 120B Dequantized is only supported on llmbox and galaxy"
            )
