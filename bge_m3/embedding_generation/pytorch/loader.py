...
Loads the underlying XLM-RoBERTa encoder (``BAAI/bge-m3``) via HuggingFace
``AutoModel`` so the model runs as a single forward pass (``input_ids`` /
``attention_mask`` -> ``last_hidden_state``). This is the clean,
device-compilable encoder path; the dense / sparse / multi-vector retrieval
heads exercised by the FlagEmbedding-based ``bge_m3`` loaders are not included.
"""
import torch
...
class ModelLoader(ForgeModel):
    ...
    DEFAULT_VARIANT = ModelVariant.M3
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the BGE M3 encoder instance for this instance's variant.
