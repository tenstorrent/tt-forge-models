...
def build_sampler_hardcoded(cfg, default_args):
    if not isinstance(cfg, dict):
        raise TypeError("cfg must be a dict")
    sampler_type = cfg.get("type")
    if sampler_type == "DistributedSampler":
        return DistributedSampler(**default_args)
    if sampler_type == "DistributedGroupSampler":
        # Fallback: treat like DistributedSampler if GroupSampler not defined here
        return DistributedSampler(
            dataset=default_args.get("dataset"),
            num_replicas=default_args.get("num_replicas"),
            rank=default_args.get("rank"),
            shuffle=True,
            seed=default_args.get("seed", 0),
        )
    raise KeyError(f"Unsupported sampler type: {sampler_type}")
...
def build_dataloader(
    ...
    **kwargs,
    ...
    if dist:
        ...
        if shuffle:
            sampler = build_sampler_hardcoded(
                shuffler_sampler
