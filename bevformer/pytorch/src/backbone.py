...
from .common_imports import BaseModule
...
class Bottleneck(BaseModule):
    ...
    def make_block_plugins(self, in_channels, plugins):
        ...
        for plugin in plugins:
            plugin = plugin.copy()
            name, layer = build_plugin_layer_hardcoded(
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        if self.deep_stem:
            self.stem = nn.Sequential(
                ...
                build_norm_hardcoded(self.norm_cfg, stem_channels // 2),
                ...
                build_norm_hardcoded(self.norm_cfg, stem_channels),
                nn.ReLU(inplace=True),
            )
        else:
            self.conv1 = nn.Conv2d(
