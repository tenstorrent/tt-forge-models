...
def build_transformer(cfg, default_args=None):
    ...
    # Bypass registry for PerceptionTransformer to remove dependency on TRANSFORMER registry
    if isinstance(cfg, dict):
        obj_type = cfg.get("type")
        if obj_type == "PerceptionTransformer":
            from transformer import PerceptionTransformer

            args = cfg.copy()
            args.pop("type", None)
            return PerceptionTransformer(**args)
