...
class BEVFormer(MVXTwoStageDetector):
    ...
    def forward_test(self, img_metas, img=None, **kwargs):
        ...
        if self.prev_frame_info["prev_bev"] is not None:
            ...
            # Force pure NumPy math to avoid Torch in-place ops during Dynamo tracing
            can_bus[:3] = np.asarray(can_bus[:3]) - np.asarray(
...
class PerceptionTransformer(BaseModule):
    ...
    def get_bev_features(
        ...
        **kwargs,
        ...
        # Build can_bus tensor on the right device/dtype once, then do all math in torch
