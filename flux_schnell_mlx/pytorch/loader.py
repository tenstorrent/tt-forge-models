...
"""
FLUX.1-schnell MLX (argmaxinc/mlx-FLUX.1-schnell) model loader implementation.

FLUX.1-schnell is a 12B parameter text-to-image generation model in MLX quantized format,
based on the FLUX transformer architecture from Black Forest Labs.

Available variants:
- FLUX1_SCHNELL_MLX: MLX quantized variant
"""
...
class ModelVariant(StrEnum):
    """Available FLUX.1-schnell MLX model variants."""

    FLUX1_SCHNELL_MLX = "flux1_schnell_mlx"


class ModelLoader(ForgeModel):
    """FLUX.1-schnell MLX model loader implementation."""

    _VARIANTS = {
        ModelVariant.FLUX1_SCHNELL_MLX: ModelConfig(
            pretrained_model_name=REPO_ID,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.FLUX1_SCHNELL_MLX
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the FLUX.1-schnell transformer from MLX checkpoint.

        Returns:
            torch.nn.Module: The FLUX transformer model instance.
        """
        if self.pipeline is None:
            self.pipeline = load_flux_mlx_pipe(REPO_ID, BASE_MODEL)
