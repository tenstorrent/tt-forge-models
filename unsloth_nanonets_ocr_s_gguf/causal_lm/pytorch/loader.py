...
"""
unsloth/Nanonets-OCR-s-GGUF model loader implementation for causal language modeling.
...
class ModelVariant(StrEnum):
    """Available unsloth/Nanonets-OCR-s-GGUF model variants for causal language modeling."""

    NANONETS_OCR_S_GGUF = "nanonets_ocr_s_gguf"


class ModelLoader(ForgeModel):
    """unsloth/Nanonets-OCR-s-GGUF model loader implementation for causal language modeling tasks.
    ...
    _VARIANTS = {
        ModelVariant.NANONETS_OCR_S_GGUF: LLMModelConfig(
            pretrained_model_name="nanonets/Nanonets-OCR-s",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.NANONETS_OCR_S_GGUF

    sample_text = (
        "Extract the text from the document as if you were reading it naturally."
    )
