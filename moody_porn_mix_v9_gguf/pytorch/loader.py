...
based on the Lumina2 architecture with a Qwen3-4B text encoder.
...
from .src.model_utils import load_lumina2_transformer, make_lumina2_inputs
...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        Returns:
            Lumina2Transformer2DModel: The loaded transformer in eval mode.
        """
        compute_dtype = dtype_override if dtype_override is not None else torch.bfloat16
        if self.transformer is None:
            self.transformer = load_lumina2_transformer(
                REPO_ID, self.GGUF_FILE, compute_dtype
            )
    ...
    def load_inputs(self, dtype_override=None):
        ...
        Returns:
            dict: Input tensors for the Lumina2 transformer.
        """
        dtype = dtype_override if dtype_override is not None else torch.bfloat16
        return make_lumina2_inputs(dtype=dtype)
