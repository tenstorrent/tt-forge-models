...
from .modeling_embedding import (
...
class PyramidFluxTransformer(ModelMixin, ConfigMixin):
    ...
    def merge_input(self, sample, encoder_hidden_length, encoder_attention_mask):
        ...
        text_ids = torch.zeros(pad_batch_size, encoder_attention_mask.shape[1], 3).to(
            device=device
        )
        input_ids_list = [
        ...
        image_rotary_emb = [
            self.pos_embed(input_ids) for input_ids in input_ids_list
    ...
    def forward(
        ...
        for i_p in range(len(hidden_length)):
            ...
            if is_sequence_parallel_initialized():
                sp_group = get_sequence_parallel_group()
                sp_group_size = get_sequence_parallel_world_size()
                batch_hidden_states[i_p] = all_to_all(
                    batch_hidden_states[i_p],
                    sp_group,
                    sp_group_size,
                    scatter_dim=0,
                    gather_dim=1,
            ...
            batch_hidden_states[i_p] = torch.cat(
                [encoder_hidden_states[i_p::num_stages], batch_hidden_states[i_p]],
                dim=1,
            )
        ...
        batch_hidden_states = list(
            torch.split(hidden_states, concat_hidden_length, dim=1)
        )
        ...
        for i_p in range(len(concat_hidden_length)):
            ...
            batch_hidden_states[i_p] = batch_hidden_states[i_p][
                :, encoder_hidden_length:, ...
            ]
