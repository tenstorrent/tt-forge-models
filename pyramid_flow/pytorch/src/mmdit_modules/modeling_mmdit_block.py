...
class VarlenFlashSelfAttentionWithT5Mask:
    ...
    def apply_rope(self, xq, xk, freqs_cis):
        ...
        return xq_out.reshape(*xq.shape).type_as(xq), xk_out.reshape(*xk.shape).type_as(
            xk
        )
    ...
    def __call__(
        ...
        for i_p, length in enumerate(hidden_length):
            ...
            if image_rotary_emb is not None:
                concat_qkv_tokens[:, :, 0], concat_qkv_tokens[:, :, 1] = (
                    self.apply_rope(
                        concat_qkv_tokens[:, :, 0],
                        concat_qkv_tokens[:, :, 1],
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.norm_add_q is not None:
            encoder_hidden_states_query_proj = self.norm_add_q(
                encoder_hidden_states_query_proj
            )

        if self.norm_add_k is not None:
            encoder_hidden_states_key_proj = self.norm_add_k(
