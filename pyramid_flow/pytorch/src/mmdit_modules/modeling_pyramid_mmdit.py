...
class PyramidDiffusionMMDiT(ModelMixin, ConfigMixin):
    ...
    def _prepare_temporal_rope_ids(
        ...
        # the end, which torch_xla rejects when these are mixed with XLA tensors
        # via the in-place add below ("Input tensor is not an XLA tensor:
        # CPUFloatType"). Mirrors the same fix applied to the miniFLUX
        # `_prepare_image_ids`.
        latent_image_ids = torch.zeros(temp, height, width, 1, device=device)
        latent_image_ids[..., 0] = (
            latent_image_ids[..., 0]
