...
def rope(pos: torch.Tensor, dim: int, theta: int) -> torch.Tensor:
    ...
    # float32 (not float64): TT/XLA does not support float64; the rope angles
    # are cast back to float() below, so float32 accumulation is sufficient.
...
class PyramidDiffusionMMDiT(ModelMixin, ConfigMixin):
    ...
    def _prepare_temporal_rope_ids(self, batch_size, temp, height, width, device, start_time_stamp=0):
        # Create position-id tensors directly on `device`. Building them on CPU
        # and `.to(device)`-ing at the end lets dynamo constant-fold a CPU
        # constant that the tt/XLA backend then strands on host (CPUFloatType in
        # the downstream `where`/attention-mask ops).
        latent_image_ids = torch.zeros(temp, height, width, 1, device=device)
        latent_image_ids[..., 0] = latent_image_ids[..., 0] + torch.arange(start_time_stamp, start_time_stamp + temp, device=device)[:, None, None]
        latent_image_ids = latent_image_ids[None, :].repeat(batch_size, 1, 1, 1, 1)
        latent_image_ids = rearrange(latent_image_ids, 'b t h w c -> b (t h w) c')
        return latent_image_ids.to(device=device)
    ...
    def merge_input(self, sample, encoder_hidden_length, encoder_attention_mask):
        ...
        if self.use_flash_attn:
            ...
            # prepare text ids (created directly on `device`; see note in
            # _prepare_temporal_rope_ids about CPU constant folding)
            text_ids = torch.arange(1, real_batch_size + 1, dtype=encoder_attention_mask.dtype, device=device).unsqueeze(1).repeat(1, encoder_hidden_length)
            text_ids = torch.where(encoder_attention_mask == 0, torch.zeros_like(text_ids), text_ids)

            # prepare image ids
            image_ids = torch.arange(1, real_batch_size + 1, dtype=encoder_attention_mask.dtype, device=device).unsqueeze(1).repeat(1, max(hidden_length))
            image_ids_list = []
            for i_p, length in enumerate(hidden_length):
