...
class PyramidDiffusionMMDiT(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if temp_pos_embed_type == "rope":
            self.temp_rope_embed = EmbedNDRoPE(
                self.inner_dim, 10000, axes_dim=[attention_head_dim]
            )
        else:
    ...
    def _prepare_temporal_rope_ids(
        ...
        # tt-xla device-correctness: build this shape-only constant directly on
        # `device` via broadcasting instead of in-place index assignment on a CPU
        # tensor. The original `zeros(...)[..., 0] = ... + arange(...)` pattern
        # decomposes to an aten.where on CPU operands, which then leaks a CPU
        # tensor into the XLA graph ("Input tensor is not an XLA tensor").
        ...
        latent_image_ids = time_ids.view(temp, 1, 1, 1).expand(temp, height, width, 1)
        latent_image_ids = latent_image_ids[None, :].repeat(batch_size, 1, 1, 1, 1)
        latent_image_ids = rearrange(latent_image_ids, "b t h w c -> b (t h w) c")
    ...
    def merge_input(self, sample, encoder_hidden_length, encoder_attention_mask):
        ...
        if self.pos_embed_type == "rope":
            ...
            text_ids = torch.zeros(pad_batch_size, encoder_hidden_length, 3).to(
                device=device
            )
            image_ids_list = self._prepare_pyramid_latent_image_ids(
                pad_batch_size, temp_list, height_list, width_list, device
            )
            input_ids_list = [
                torch.cat([text_ids, image_ids], dim=1) for image_ids in image_ids_list
            ]
            image_rotary_emb = [
                self.rope_embed(input_ids) for input_ids in input_ids_list
            ...
            if self.temp_pos_embed_type == "rope" and self.add_temp_pos_embed:
                ...
                if is_sequence_parallel_initialized():
                    ...
                    image_rotary_emb = [
                        all_to_all(
                            x_.repeat(1, 1, sp_group_size, 1, 1, 1),
                            sp_group,
                            sp_group_size,
                            scatter_dim=2,
                            gather_dim=0,
                        ...
                        for x_ in image_rotary_emb
                    ]
                    input_ids_list = [
                        all_to_all(
                            input_ids.repeat(1, 1, sp_group_size),
                            sp_group,
                            sp_group_size,
                            scatter_dim=2,
                            gather_dim=0,
        ...
        if self.use_flash_attn:
            ...
            for i_p, length in enumerate(hidden_length):
                ...
                if is_sequence_parallel_initialized():
                    sp_group = get_sequence_parallel_group()
                    sp_group_size = get_sequence_parallel_world_size()
                    pad_attention_mask = all_to_all(
                        pad_attention_mask.unsqueeze(2).repeat(1, 1, sp_group_size),
                        sp_group,
                        sp_group_size,
                        scatter_dim=2,
                        gather_dim=0,
            ...
            if is_sequence_parallel_initialized():
                ...
                concat_output = True if self.training else False
                text_ids = all_to_all(
                    text_ids.unsqueeze(2).repeat(1, 1, sp_group_size),
                    sp_group,
                    sp_group_size,
                    scatter_dim=2,
                    gather_dim=0,
                ...
                image_ids_list = [
                    all_to_all(
                        image_ids_.unsqueeze(2).repeat(1, 1, sp_group_size),
                        sp_group,
                        sp_group_size,
                        scatter_dim=2,
                        gather_dim=0,
                    ...
                    for image_ids_ in image_ids_list
                ]
        ...
        for i_p, length in enumerate(hidden_length):
            ...
            if is_sequence_parallel_initialized():
                ...
                hidden_states = all_to_all(
                    hidden_states, sp_group, sp_group_size, scatter_dim=0, gather_dim=1
                )
    ...
    def forward(
        ...
        if is_sequence_parallel_initialized():
            ...
            for i_p, hidden_states_ in enumerate(hidden_states):
                assert (
                    hidden_states_.shape[1] % sp_group_size == 0
                ), "The sequence length should be divided by sequence parallel size"
                hidden_states_ = all_to_all(
                    hidden_states_,
                    sp_group,
                    sp_group_size,
                    scatter_dim=1,
            ...
            temb = all_to_all(
                temb.unsqueeze(1).repeat(1, sp_group_size, 1),
                sp_group,
                sp_group_size,
                scatter_dim=1,
                gather_dim=0,
        ...
        for i_b, block in enumerate(self.transformer_blocks):
            if (
                ...
                and (
                    i_b
                    >= int(
                        len(self.transformer_blocks) * self.gradient_checkpointing_ratio
                    )
                )
            ):
