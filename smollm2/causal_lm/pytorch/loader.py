...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the SmolLM2 model for causal LM.

        Args:
            dtype_override: Optional torch dtype (e.g. torch.bfloat16).
        Returns:
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        Args:
            dtype_override: Optional dtype override (not applied to token ids).
            batch_size: Batch size (default 1).
        Returns:
            dict: Input tensors suitable for causal LM forward pass.
        """
        if self.tokenizer is None:
            self._load_tokenizer()
