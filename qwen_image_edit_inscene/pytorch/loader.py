...
- QWEN_IMAGE_EDIT_INSCENE_ANNOTATE: base model only; InScene-Annotate-0.4 LoRA
  was trained on a 5120-dim model, incompatible with the current 3072-dim
  Qwen/Qwen-Image-Edit release.
"""
...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override: Optional[torch.dtype] = None, **kwargs):
        ...
        Loads the full pipeline, applies the diffusers-format LoRA (InScene
        variant only), fuses the weights into the transformer, and returns
        the transformer component.

        Returns:
            QwenImageTransformer2DModel with LoRA weights fused (InScene) or
            without LoRA (Annotate, due to dimension incompatibility).
        """
        ...
        if self._variant == ModelVariant.QWEN_IMAGE_EDIT_INSCENE:
            pipe.load_lora_weights(LORA_REPO, weight_name=LORA_WEIGHT_INSCENE)
            pipe.fuse_lora()
