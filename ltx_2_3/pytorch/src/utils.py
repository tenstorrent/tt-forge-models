...
def transformer_video_inputs(
    ...
    video = Modality(
        ...
        # (B, T, 1): per-token sigma. X0Model.to_denoised does
        # `latent - velocity * timesteps`, so timesteps must broadcast against
        # the (B, T, D) velocity — a bare (B, T) only works by accident when
        # the token count T equals the latent channel count D (=128).
        timesteps=torch.ones(b, num_tokens, 1, dtype=dtype),
        positions=positions,
