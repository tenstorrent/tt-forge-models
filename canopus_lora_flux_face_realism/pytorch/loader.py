...
"""
Canopus LoRA FLUX Face Realism (prithivMLmods/Canopus-LoRA-Flux-FaceRealism)
model loader implementation.

Canopus-LoRA-Flux-FaceRealism is a LoRA fine-tune of FLUX.1-dev specialized
in photorealistic portraits and face-focused imagery.

Available variants:
- FACE_REALISM: prithivMLmods/Canopus-LoRA-Flux-FaceRealism LoRA text-to-image generation
"""
...
LORA_WEIGHT_NAME = "Canopus-LoRA-Flux-FaceRealism.safetensors"


class ModelVariant(StrEnum):
    """Available Canopus LoRA FLUX Face Realism model variants."""

    FACE_REALISM = "Face-Realism"


class ModelLoader(ForgeModel):
    """Canopus LoRA FLUX Face Realism model loader implementation."""

    _VARIANTS = {
        ModelVariant.FACE_REALISM: ModelConfig(
            pretrained_model_name=LORA_REPO,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.FACE_REALISM
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        ...
        return ModelInfo(
            model="Canopus-LoRA-Flux-FaceRealism",
            variant=variant,
    ...
    def _load_pipeline(self, dtype_override=None):
        """Load FLUX.1-dev pipeline with Canopus Face Realism LoRA weights.

        Args:
            dtype_override: Optional torch.dtype to override the model's default dtype.

        Returns:
            The loaded pipeline instance with LoRA weights applied
        """
        pipe_kwargs = {
        ...
        self.pipe = FluxPipeline.from_pretrained(BASE_MODEL, **pipe_kwargs)
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the FLUX transformer model with LoRA weights applied.

        Args:
            dtype_override: Optional torch.dtype to override the model's default dtype.

        Returns:
            torch.nn.Module: The FLUX transformer model instance with LoRA applied.
        """
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the Canopus Face Realism model.
        ...
        prompt = (
            "Headshot of handsome young man, wearing dark gray sweater with buttons "
            "and big shawl collar, brown hair and short beard, serious look on his "
            "face, black background, soft studio lighting, portrait photography, "
            "face realism"
        )
