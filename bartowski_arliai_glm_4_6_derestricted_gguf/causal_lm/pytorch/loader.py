...
"""
bartowski ArliAI GLM-4.6-Derestricted GGUF model loader implementation for causal language modeling.
"""
import torch
...
def _patch_transformers_glm4moe_gguf():
    ...
    GGUF loading support for the "glm4moe" architecture name used in GGUF files.
    The gguf library already knows about glm4moe tensor names; we only need to
    bridge transformers' config/tensor processing layer.
    """
    ...
    # 2. Add config mapping for glm4moe -> Glm4MoeConfig field names
    ...
    #    "glm4_moe". Glm4MoeConfig also expects rope_theta inside a
    #    rope_parameters dict, not as a top-level key.
...
class ModelVariant(StrEnum):
    """Available bartowski ArliAI GLM-4.6-Derestricted GGUF model variants for causal language modeling."""

    ARLIAI_GLM_4_6_DERESTRICTED_GGUF = "ArliAI_GLM_4_6_Derestricted_GGUF"


class ModelLoader(ForgeModel):
    """bartowski ArliAI GLM-4.6-Derestricted GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.ARLIAI_GLM_4_6_DERESTRICTED_GGUF: LLMModelConfig(
            pretrained_model_name="bartowski/ArliAI_GLM-4.6-Derestricted-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.ARLIAI_GLM_4_6_DERESTRICTED_GGUF

    GGUF_FILE = "ArliAI_GLM-4.6-Derestricted-Q4_K_M/ArliAI_GLM-4.6-Derestricted-Q4_K_M-00001-of-00006.gguf"
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="bartowski ArliAI GLM-4.6-Derestricted GGUF",
            variant=variant,
    ...
    def load_shard_spec(self, model):
        ...
        for layer in model.model.layers:
            ...
            if hasattr(mlp, "shared_experts"):
                ...
                shard_specs[mlp.shared_experts.gate_proj.weight] = (
                    "model",
                    "batch",
                )
                shard_specs[mlp.shared_experts.down_proj.weight] = (
                    "batch",
                    "model",
                )
