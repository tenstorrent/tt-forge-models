...
class ModelLoader(ForgeModel):
    ...
    def _latent_dims():
        ...
        audio_num_frames = 9
        # One audio token per latent audio frame: ``audio_rope.prepare_audio_coords``
        # produces exactly ``audio_num_frames`` RoPE patches, and the split-rotary
        # attention reshapes the audio query against that count, so the packed
        # ``audio_hidden_states`` sequence length must equal ``audio_num_frames``.
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        if v == ModelVariant.LTX2_TRANSFORMER:
            ...
            return [
                ...
                # timestep is one scalar per batch element (NOT per-token): the
                # real LTX2Pipeline passes ``t.expand(batch_size)`` so the shared
                # video/audio time embedding broadcasts across each modality's
                # token count. A per-video-token timestep wrongly forces the
                # audio path to 2 tokens vs its 144 -> dim-1 mismatch.
