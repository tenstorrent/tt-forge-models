...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.AMLM_HARD: LLMModelConfig(
            pretrained_model_name="leukas/amlm_hard",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.AMLM_HARD

    def __init__(self, variant=None):
        super().__init__(variant)
        self.model_name = self._variant_config.pretrained_model_name
        self.max_length = self._variant_config.max_length
        self.sample_text = "Paris is the [MASK] of France."
        self.tokenizer = None
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        # AutoTokenizer → DebertaV2TokenizerFast has cls.model=Unigram but the repo ships a BPE tokenizer.json, causing KeyError: 0 in convert_to_native_format.
