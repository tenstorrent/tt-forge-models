    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower_aux_list"):
            ...
            if projector_type == 'sva':
                ...
                # Wrap in nn.ModuleList so that from_pretrained can navigate the
                # module hierarchy and load vision-tower weights from the Cambrian
                # checkpoint.  A plain Python list leaves the modules unregistered,
                # so their weights stay on meta and never get materialized.
                self.vision_tower_aux_list = nn.ModuleList(build_vision_tower_aux_list(config, delay_load=True))
...
class CambrianMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_for_generation(
        ...
        # Move to CPU before .tolist() to avoid aten._local_scalar_dense graph break
        # on XLA/TT backends (device-to-host transfer not supported during compilation).
        image_pos = input_ids[0].cpu().eq(IMAGE_TOKEN_INDEX).nonzero(as_tuple=False).flatten().tolist()
