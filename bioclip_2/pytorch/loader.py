...
class ModelLoader(ForgeModel):
    """BioCLIP-2 model loader using OpenCLIP for biological image-text similarity."""
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the BioCLIP-2 model instance.

        Args:
            dtype_override: Optional torch.dtype to override the model's default dtype.

        Returns:
            torch.nn.Module: The BioCLIP-2 model instance.
        """
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the BioCLIP-2 model.
    ...
    def post_process(self, outputs):
        """Post-process BioCLIP-2 model outputs to extract similarity scores.
