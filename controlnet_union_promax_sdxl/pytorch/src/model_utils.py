...
def load_controlnet_union_promax_sdxl_pipe(controlnet_model_name, base_model_name):
    """Load ControlNet Union ProMax SDXL pipeline.
...
def create_union_conditioning_image(height=1024, width=1024):
    """Create a dummy conditioning image for ControlNet Union.

    Creates a simple synthetic image suitable for ControlNet Union conditioning.
...
def controlnet_union_promax_sdxl_preprocessing(
    pipe,
    prompt,
    control_image,
    device="cpu",
    ...
    """Preprocess inputs for ControlNet Union ProMax SDXL model.

    Args:
        pipe: StableDiffusionXLControlNetUnionPipeline
        prompt: Text prompt for generation
        control_image: Conditioning image
        ...
        controlnet_conditioning_scale: ControlNet conditioning scale (default: 0.5)
        control_mode: Control mode index for ControlNet Union (default: 0 for openpose)
