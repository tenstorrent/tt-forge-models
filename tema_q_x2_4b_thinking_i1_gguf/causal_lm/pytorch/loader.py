...
def _refresh_gguf_availability():
    """Refresh transformers' package distribution mapping so gguf version is found correctly.

    gguf has no __version__ attribute, so transformers falls back to importlib.metadata.
    PACKAGE_DISTRIBUTION_MAPPING must be refreshed after gguf is installed at test time,
    and the lru_cache on is_gguf_available must be cleared so it re-evaluates.
    """
