...
"""
DevQuasar GLM-4.7 GGUF model loader implementation for causal language modeling.
"""
import torch
...
def _patch_transformers_glm4moe_gguf():
    ...
    The GLM-4.7 model uses the 'glm4moe' architecture identifier in its GGUF
    metadata. Transformers has Glm4MoeForCausalLM (model_type='glm4_moe') but
    lacks GGUF loading support for the glm4moe architecture. We bridge the gap
    by registering the config mapping and remapping model_type to glm4_moe.
    """
    ...
    # 2. Add config mapping for glm4moe -> glm4_moe transformers config fields
    ...
    # 3. Register glm4moe tokenizer converter with merge-fixing logic.
    # GLM vocabularies contain tokens with literal spaces, so the
    ...
    # 5. Patch get_gguf_hf_weights_map to remap glm4_moe -> glm4moe for gguf-py arch lookup
...
class ModelVariant(StrEnum):
    """Available DevQuasar GLM-4.7 GGUF model variants for causal language modeling."""

    GLM_4_7_Q2_K_GGUF = "4.7_Q2_K_GGUF"


class ModelLoader(ForgeModel):
    """DevQuasar GLM-4.7 GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.GLM_4_7_Q2_K_GGUF: LLMModelConfig(
            pretrained_model_name="DevQuasar/zai-org.GLM-4.7-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.GLM_4_7_Q2_K_GGUF

    GGUF_FILE = "zai-org.GLM-4.7.Q2_K-00001-of-00009.gguf"
