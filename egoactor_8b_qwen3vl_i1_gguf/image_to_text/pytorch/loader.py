...
Note: The qwen3vl GGUF architecture is not yet fully supported by transformers'
GGUF tensor name mapping, so we load from the base HF safetensors checkpoint.
"""
...
class ModelLoader(ForgeModel):
    ...
    Note: Uses the base safetensors checkpoint instead of GGUF because the
    qwen3vl GGUF tensor name mapping is not yet supported by transformers.
    """

    _VARIANTS = {
        ModelVariant.EGOACTOR_8B_QWEN3VL_I1_GGUF: LLMModelConfig(
            pretrained_model_name="BAAI-Agents/EgoActor-8b-Qwen3VL",
            max_length=128,
        ),
    }
