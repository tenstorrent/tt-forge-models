...
class _STFTFn(nn.Module):
    ...
    def forward(self, y: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # STFT-as-conv reformulation (TT bring-up patch). The upstream
        # ``F.conv1d`` frames ``y`` into ``win_length`` windows at stride
        # ``hop_length`` and dot-products each window against the 514 DFT x Hann
        # basis rows. TTNN lowers that strided conv into a ``ttnn.conv2d`` over
        # the full ~21k-sample width whose L1 footprint overflows (fails
        # identically in fp32 and bf16 -- "OutOfMemoryError - Not enough L1
        # memory", no fallback config works). The effective compute is tiny: one
        ...
        # cross-correlation (no kernel flip) and unfold preserves window order,
        # so this is numerically identical to the original conv.
        frames = F.unfold(
            y.unsqueeze(2),  # (B, 1, 1, T_pad)
            kernel_size=(1, self.win_length),
            stride=(1, self.hop_length),
        )  # (B, win_length, n_frames)
        basis = self.forward_basis.reshape(
            self.forward_basis.shape[0], self.win_length
        )
        spec = torch.matmul(basis.to(frames.dtype), frames)  # (B, n_freqs*2, n_frames)
...
class VocoderWithBWE(nn.Module):
    ...
    def forward(self, mel_spec: torch.Tensor) -> torch.Tensor:
        ...
        # Forge: the loader may stamp ``_forge_compute_dtype`` (e.g. bfloat16) to
        # run the whole forward in the weight dtype instead of fp32. fp32 is ideal
        # for spectral quality but its activation footprint overflows Tenstorrent
        # L1 on this conv stack, and CPU autocast-to-fp32 is unsupported. When the
        # flag is bf16/fp16, autocast is disabled and the input is cast to match
        # the weights so the graph is dtype-consistent on device.
        compute_dtype = getattr(self, "_forge_compute_dtype", torch.float32)
