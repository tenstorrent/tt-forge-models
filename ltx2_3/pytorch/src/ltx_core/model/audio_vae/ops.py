...
# tt-forge bringup: ``torchaudio``'s compiled extension is ABI-incompatible with
# the custom Tenstorrent torch build, so the top-level import is removed and
# deferred into ``AudioProcessor`` (the only consumer). The AudioVAE encode/decode
# paths operate on mel/latent tensors via ``PerChannelStatistics`` and never touch
# torchaudio, so the component tests import and run without it.
...
class AudioProcessor(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        import torchaudio  # deferred: see module-level note
