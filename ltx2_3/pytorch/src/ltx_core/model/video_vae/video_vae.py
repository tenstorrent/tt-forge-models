...
class VideoDecoder(nn.Module):
    ...
    def forward(
        ...
        # tt-forge bringup: ``next(self.parameters())`` is not traceable by
        # torch.compile/dynamo (raises a free-variable NameError). When the
        # loader has stamped the weight dtype as a plain attribute, read that
        # constant instead; otherwise fall back to the original eager lookup so
        # standalone use is unchanged.
        weights_dtype = getattr(self, "_forge_weights_dtype", None)
