...
def _fix_gguf_version_detection():
    """Ensure gguf version is discoverable even when installed after transformers import.

    transformers.utils.import_utils.PACKAGE_DISTRIBUTION_MAPPING is computed once
    at module-import time. When gguf is installed later (e.g. by RequirementsManager),
    the mapping is stale and the fallback path reads gguf.__version__ which is 'N/A',
    causing packaging.version.InvalidVersion. Refreshing the mapping entry and setting
    __version__ on the gguf module fixes this.
    """
    try:
        ...
        if not hasattr(_gguf_mod, "__version__") or _gguf_mod.__version__ == "N/A":
            _gguf_mod.__version__ = gguf_ver
