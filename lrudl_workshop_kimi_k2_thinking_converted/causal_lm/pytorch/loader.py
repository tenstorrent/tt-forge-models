...
"""
lrudl-workshop/Kimi-K2-Thinking-converted model loader implementation for
causal language modeling.

The repository ships a compressed-tensors quantized copy of Moonshot AI's
Kimi-K2-Thinking built on the DeepSeek V3 architecture, reusing the same
``modeling_deepseek``/``configuration_deepseek`` remote code.
"""
import os
import sys
...
class ModelVariant(StrEnum):
    """Available lrudl-workshop Kimi-K2-Thinking-converted model variants."""

    KIMI_K2_THINKING_CONVERTED = "Kimi-K2-Thinking-converted"


class ModelLoader(ForgeModel):
    """lrudl-workshop Kimi-K2-Thinking-converted loader for causal language modeling."""

    _VARIANTS = {
        ModelVariant.KIMI_K2_THINKING_CONVERTED: None,
    }

    DEFAULT_VARIANT = ModelVariant.KIMI_K2_THINKING_CONVERTED
    ...
    def __init__(
        ...
        self.model_name = "lrudl-workshop/Kimi-K2-Thinking-converted"
        self.tokenizer = None
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="Kimi-K2-Thinking-converted",
            variant=variant,
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the Kimi-K2-Thinking-converted model with reduced config.

        The full model is a 1T-parameter MoE causal language model built on
        the DeepSeek V3 architecture (compressed-tensors quantized). We load
        with a reduced configuration for testing purposes.
