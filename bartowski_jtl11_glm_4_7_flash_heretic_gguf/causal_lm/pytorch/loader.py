...
"""
bartowski jtl11 GLM-4.7-Flash-heretic GGUF model loader implementation for causal language modeling.
"""
import torch
...
class ModelVariant(StrEnum):
    """Available bartowski jtl11 GLM-4.7-Flash-heretic GGUF model variants for causal language modeling."""

    BARTOWSKI_JTL11_GLM_4_7_FLASH_HERETIC_GGUF = "jtl11_GLM_4_7_Flash_heretic_GGUF"


class ModelLoader(ForgeModel):
    """bartowski jtl11 GLM-4.7-Flash-heretic GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.BARTOWSKI_JTL11_GLM_4_7_FLASH_HERETIC_GGUF: LLMModelConfig(
            pretrained_model_name="bartowski/jtl11_GLM-4.7-Flash-heretic-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.BARTOWSKI_JTL11_GLM_4_7_FLASH_HERETIC_GGUF
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="bartowski jtl11 GLM-4.7-Flash-heretic GGUF",
            variant=variant,
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        # Load config from GGUF and fix qk_nope_head_dim.
        # In deepseek2 GGUF, attention.key_length_mla stores the total key head
        # dimension (qk_nope_head_dim + qk_rope_head_dim), but the config mapping
        # assigns it directly to qk_nope_head_dim. Subtract qk_rope_head_dim to
        # get the correct value, so q_b_proj shapes match the GGUF weights.
