...
from .configuration_audio import (
...
class LanguageBindAudioTextEmbeddings(CLIPTextEmbeddings):
    """CLIPTextEmbeddings subclass that computes position_ids on the fly.

    In transformers 5.x, position_ids is a non-persistent buffer initialised with
    expand(), whose backing storage can become invalid after model loading.
    Computing position_ids fresh from arange() in every forward pass avoids this.
    """
...
class CLIPTextTransformer(nn.Module):
    def __init__(self, config: CLIPTextConfig):
        ...
        self.embeddings = LanguageBindAudioTextEmbeddings(config)
...
class LanguageBindAudio(CLIPPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=CLIPOutput, config_class=LanguageBindAudioConfig
    )
    def forward(
        self,
