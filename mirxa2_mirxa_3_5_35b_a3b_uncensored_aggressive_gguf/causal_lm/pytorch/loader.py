...
def _patch_transformers_qwen35moe_gguf():
    """Register qwen35moe as a supported GGUF architecture.

    Qwen 3.5 35B-A3B uses qwen35moe GGUF architecture. The GGUF files store
    expert tensors as separate ffn_gate_exps/ffn_up_exps (qwen3moe naming
    convention), so we redirect the tensor name lookup to qwen3moe.
    """
    ...
    # GGUF files use qwen3moe tensor naming (separate ffn_gate_exps/ffn_up_exps)
    # rather than the combined ffn_gate_up_exps that qwen35moe gguf-py expects.
