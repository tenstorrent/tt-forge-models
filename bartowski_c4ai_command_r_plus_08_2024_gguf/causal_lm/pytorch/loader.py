...
def _patch_transformers_command_r_gguf():
    ...
    The C4AI Command R+ model uses 'command-r' as the GGUF architecture identifier,
    but transformers 5.x uses 'cohere' as the model_type for CohereForCausalLM.
    We bridge the gap by registering the config mapping and remapping model_type.
    """
    ...
    # Register command-r as a supported architecture
    GGUF_SUPPORTED_ARCHITECTURES.append("command-r")

    # Add config mapping for command-r -> cohere config fields
    ...
    # Register tokenizer converters for both pre- and post-remap architecture names
    for arch in ("command-r", "cohere"):
    ...
    def patched_load_gguf_checkpoint(*args, **kwargs):
        ...
        if config.get("model_type") == "command-r":
            ...
            # CohereConfig expects rope_parameters dict, not flat rope_theta
