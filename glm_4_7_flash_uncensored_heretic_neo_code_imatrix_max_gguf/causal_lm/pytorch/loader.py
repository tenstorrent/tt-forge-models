...
def _patch_transformers_deepseek2_gguf():
    """Patch transformers to add deepseek2 GGUF architecture support.

    The gguf library knows about deepseek2 tensor names, but transformers
    lacks the config mapping and architecture registration needed to load
    deepseek2 GGUF checkpoints.

    We keep model_type as "deepseek2" (the gguf arch name) so that
    get_gguf_hf_weights_map can find the right tensor name mapping.
    We register "deepseek2" in CONFIG_MAPPING so AutoConfig recognizes it.
    """
    ...
    # 4. Register "deepseek2" in CONFIG_MAPPING so AutoConfig.from_pretrained
    # and AutoConfig.for_model accept model_type="deepseek2" checkpoints.
    # We map to DeepseekV2Config (the HF class for this architecture).
    # model_type stays "deepseek2" so get_gguf_hf_weights_map can find the
    # correct GGUF tensor name mapping via MODEL_ARCH_NAMES["deepseek2"].
    from transformers.models.auto.configuration_auto import CONFIG_MAPPING

    if "deepseek2" not in CONFIG_MAPPING:
        CONFIG_MAPPING._extra_content["deepseek2"] = CONFIG_MAPPING["deepseek_v2"]
