...
def _patch_transformers_deepseek2_gguf():
    """Monkey-patch transformers to support deepseek2/deepseek_v2 GGUF loading.

    GLM-4.7 Flash uses the deepseek2 architecture in GGUF metadata. Transformers
    maps model_type to deepseek_v2 but lacks deepseek_v2 in GGUF_TO_FAST_CONVERTERS,
    so tokenizer loading fails. This patch bridges both gaps.
    """
    ...
    if not getattr(gguf_utils.load_gguf_checkpoint, "_deepseek2_patched", False):
