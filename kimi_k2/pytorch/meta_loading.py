...
def resolve_hf_shards_for_layers(
    ...
    *,
    ...
    Args:
        repo_id: HF Hub repo, e.g. ``"unsloth/Kimi-K2-Base-BF16"``.
    ...
    """
    print(
        f"[meta_loading] Resolving shards for {repo_id} (first {n_layers} layer(s))",
        flush=True,
    )
    try:
        ...
        print(
            f"[meta_loading] No index.json; downloading single model.safetensors",
            flush=True,
        )
    ...
    print(
        f"[meta_loading] Need {total} shard(s) for first {n_layers} layer(s)",
        flush=True,
    )
    paths: List[str] = []
    for i, s in enumerate(sorted_shards, start=1):
        print(f"[meta_loading] Downloading shard {i}/{total}: {s}", flush=True)
...
def load_model_from_checkpoint(
    ...
    *,
    ...
    Args:
        ...
        checkpoint: One of:
            * an HF Hub repo id (e.g. ``"unsloth/Kimi-K2-Base-BF16"``): only the
