...
def _ensure_hunyuan3d_importable():
    ...
    if "hy3dgen" not in sys.modules:
        ...
        # Register intermediate packages as stubs to avoid their __init__.py
        # files, which import heavy dependencies (trimesh, pymeshlab, etc.)
        # that are not needed for the DiT denoiser model.
        base = os.path.join(HUNYUAN3D_REPO_PATH, "hy3dgen")
        stub_packages = {
            "hy3dgen": base,
            "hy3dgen.shapegen": os.path.join(base, "shapegen"),
            "hy3dgen.shapegen.models": os.path.join(base, "shapegen", "models"),
        }
