...
class ModelVariant(StrEnum):
    """Available ColBERT-XM model variants for embedding generation."""

    ANTOINELOUIS_COLBERT_XM = "antoinelouis/colbert-xm"
...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.ANTOINELOUIS_COLBERT_XM: LLMModelConfig(
            pretrained_model_name="antoinelouis/colbert-xm",
            max_length=32,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.ANTOINELOUIS_COLBERT_XM
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load ColBERT-XM model from Hugging Face.

        Args:
            dtype_override: Optional torch.dtype to override the model's default dtype.

        Returns:
            torch.nn.Module: The ColBERT-XM model instance.
        """
        if self.tokenizer is None:
