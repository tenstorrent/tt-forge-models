...
class ModelLoader(ForgeModel):
    ...
    DEFAULT_VARIANT = ModelVariant.BLIP
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        ...
        if task == BlipTask.TEXT:
            ...
            # Returning text and image embeddings; treat as image-text similarity
    ...
    def _make_text_batch(self, batch_size: int) -> List[str]:
        """Create a default batch of text prompts."""
        seed_texts = ["cats sleeping", "snowy weather"]
        if batch_size <= len(seed_texts):
            return seed_texts[:batch_size]
        repeats = (batch_size + len(seed_texts) - 1) // len(seed_texts)
        return (seed_texts * repeats)[:batch_size]
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        # Use a sample image, same as tests
        image_path = get_file("http://images.cocodataset.org/val2017/000000039769.jpg")
        image = Image.open(str(image_path))

        if task == BlipTask.TEXT:
            texts = self._make_text_batch(batch_size)
            enc = processor(text=texts, return_tensors="pd", padding=True)
        ...
        texts = self._make_text_batch(batch_size)
        enc = processor(images=image, text=texts, return_tensors="pd", padding=True)
        return [enc["input_ids"], enc["pixel_values"], enc["attention_mask"]]
