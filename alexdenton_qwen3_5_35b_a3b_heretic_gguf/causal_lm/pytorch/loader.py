...
def _patch_transformers_qwen35moe_gguf():
    ...
    Qwen3.5-35B-A3B GGUF files (Q4_K_M) store expert weights as separate
    ffn_gate_exps / ffn_up_exps tensors. We route get_gguf_hf_weights_map
    through the qwen3moe gguf-py arch so the fallback mapping correctly
    merges those into the HF gate_up_proj parameter.
    """
    ...
    # Use qwen3moe gguf-py arch so separate ffn_gate_exps/ffn_up_exps tensors
    # are correctly merged into the HF gate_up_proj parameter via fallback mapping.
