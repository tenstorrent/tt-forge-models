    ...
    def load_shard_spec(self, model):
        ...
        for layer in model.model.layers:
            ...
            if hasattr(layer, "self_attn"):
                ...
                # "model" and contract it back on out_proj; dt_bias / A_log
                # follow the heads.
                la = layer.linear_attn
                shard_specs[la.in_proj_qkv.weight] = ("model", "batch")
                if hasattr(la, "conv1d"):
                    # Depthwise conv (groups=channels) is channel-parallel, so
                    # shard its weight on the channel dim. With in_proj_qkv also
                    # channel-sharded, the conv now runs on channels/model-size
                    # per core instead of all 8192 replicated (which overflowed
                    # L1: op_slicing could only width-slice and never fit). The
                    # conv OUTPUT is still gathered to replicated by
                    # load_activation_shard_spec before the fused-qkv split, so
                    # the split stays correct.
