...
"""
SAM-HQ (Segment Anything Model in High Quality) loader implementation
"""
...
class ModelVariant(StrEnum):
    """Available SAM-HQ model variants."""

    VIT_BASE = "Vit_Base"


class ModelLoader(ForgeModel):
    """SAM-HQ model loader implementation."""

    _VARIANTS = {
        ModelVariant.VIT_BASE: ModelConfig(
            pretrained_model_name="syscv-community/sam-hq-vit-base",
        ),
    }
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the SAM-HQ model instance for this instance's variant."""
        model_name = self._variant_config.pretrained_model_name

        framework_model = SamHQModel.from_pretrained(model_name, **kwargs).to("cpu")

        self.processor = SamHQProcessor.from_pretrained(model_name, **kwargs)
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the SAM-HQ model."""
        if self.processor is None:
            model_name = self._variant_config.pretrained_model_name
            self.processor = SamHQProcessor.from_pretrained(model_name)
