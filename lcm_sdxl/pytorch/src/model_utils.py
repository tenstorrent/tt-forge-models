...
"""
Helper functions for LCM-SDXL model loading and processing.
"""
...
def load_pipe(unet_id, base_model_id=BASE_MODEL_ID):
    """Load SDXL pipeline using the LCM-distilled UNet.

    Args:
        unet_id: HuggingFace model ID for the LCM-distilled UNet checkpoint.
        base_model_id: HuggingFace model ID for the base SDXL model.

    Returns:
        DiffusionPipeline: Pipeline with the LCM UNet and LCMScheduler.
    """
    unet = UNet2DConditionModel.from_pretrained(unet_id, torch_dtype=torch.float32)
