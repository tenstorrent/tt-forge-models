...
"""
Helper functions for Noob SDXL ControlNet Scribble PidiNet model loading and processing.
"""
...
def load_controlnet_scribble_pidinet_sdxl_pipe(controlnet_model_name, base_model_name):
    """Load ControlNet Scribble PidiNet SDXL pipeline.
...
def create_scribble_pidinet_conditioning_image(height=1024, width=1024):
    """Create a dummy scribble PidiNet conditioning image.
...
def controlnet_scribble_pidinet_sdxl_preprocessing(
    pipe,
    prompt,
    control_image,
    device="cpu",
    ...
    """Preprocess inputs for ControlNet Scribble PidiNet SDXL model.

    Args:
        pipe: StableDiffusionXLControlNetPipeline
        prompt: Text prompt for generation
        control_image: Scribble PidiNet conditioning image
    ...
    down_block_additional_residuals, mid_block_additional_residual = pipe.controlnet(
        ...
        timesteps[0].to(controlnet_dtype),
        ...
        added_cond_kwargs={
            k: v.to(controlnet_dtype) if isinstance(v, torch.Tensor) else v
