...
"""
mlx-community/Qwen3-ASR-0.6B-4bit model loader implementation for speech recognition (ASR).

mlx-community/Qwen3-ASR-0.6B-4bit is a 4-bit MLX-quantized conversion of
...
class ModelVariant(StrEnum):
    """Available mlx-community/Qwen3-ASR-0.6B-4bit speech recognition model variants."""

    V0_6B_4BIT = "0.6B_4bit"
...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.V0_6B_4BIT: ModelConfig(
            pretrained_model_name="mlx-community/Qwen3-ASR-0.6B-4bit",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.V0_6B_4BIT
    ...
    def _load_model_wrapper(self, dtype_override=None):
        ...
        # mlx-community/Qwen3-ASR-0.6B-4bit config.json has quantization_config
        # without quant_method; transformers >=4.57 raises ValueError on this.
        # Strip it so the model loads as standard floats.
    ...
    def load_inputs(self, dtype_override=None):
        ...
        # Cast float tensors to match the model dtype so Conv layers don't
        # get a float/bfloat16 mismatch.
        if dtype_override is not None:
            inputs = {
