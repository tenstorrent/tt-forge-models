...
def _find_real_load_gguf():
    ...
    others use a closure variable. We BFS both paths to find the first function
    whose signature explicitly accepts model_to_load.
    """
    import inspect
...
def _patch_qwen3vl_for_tt_device(model=None):
    ...
    sync that fails with Error code: 13. Moving metadata tensors to CPU before
    the .tolist() calls avoids the sync while keeping all actual computations
    on TT device.
    """
    try:
...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        # Load config from the base model (has correct nested text_config /
        # vision_config for Qwen3VLForConditionalGeneration). Passing config
        # explicitly skips GGUF config parsing which would map flat fields to
        # the wrong struct level.
        config = Qwen3VLConfig.from_pretrained(self.BASE_MODEL)

        # ignore_mismatched_sizes: the GGUF only contains LM backbone weights.
        # The recursive get_gguf_hf_weights_map call dives into the visual
        # merger sub-module where 'norm' spuriously claims the 'output_norm'
        # GGUF key (which belongs to the LM backbone), producing a size mismatch
        # (4096 vs 1152). Allowing the mismatch re-inits that visual parameter
        # from the model default; both CPU and TT use the same weights.
