...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        # For config loading (return_tensors=False): delegate to the existing
        # chain (safe for narrow-sig callers).
        ...
        def _robobrain_load_gguf(*args, **kw):
            ...
            if result.get("config", {}).get("model_type") == "qwen3vl":
                ...
                # Fix vision_config out_hidden_size to match text hidden_size
                config.setdefault("vision_config", {})["out_hidden_size"] = (
                    text_config.get("hidden_size", 2048)
                )
        ...
        gguf_utils.load_gguf_checkpoint = _robobrain_load_gguf
        for mod in (config_utils, modeling_utils):
            if hasattr(mod, "load_gguf_checkpoint"):
                mod.load_gguf_checkpoint = _robobrain_load_gguf
