...
def _dequantize_bnb4_to_bf16(model):
    ...
    for name, module in replacements:
        ...
        if qs is not None:
            dq_weight = bnb.functional.dequantize_4bit(w.data, qs).to(torch.bfloat16)
        else:
            dq_weight = w.data.to(torch.bfloat16)
...
def _patch_mistral3_for_tt_device(model):
    ...
    # Fix 1: split_sizes uses image_sizes on TT device → bfloat16 int64 rounding
    # (prod(2310) rounds to 2320). Compute split_sizes on CPU instead.
    ...
    # Fix 2: generate_block_attention_mask uses in-place slice assignment on TT tensor.
    # Replace globally with a CPU-built version that moves to device at the end.
    try:
        ...
        _pixtral_mod.generate_block_attention_mask = _generate_block_attention_mask_cpu
    except Exception:
        pass

    # Fix 3: masked_scatter cumsum OOM (seq_len × hidden_size cumsum → 45 GB on TT).
    # Replace with float32 one-hot matmul gather — no int64, no large cumsum.
    _original_forward = inner.__class__.forward
    ...
    def _patched_forward(
        ...
        **kwargs,
        ...
        if pixel_values is not None:
            ...
            # Token-level gather via float32 one-hot matmul — avoids masked_scatter OOM.
            # Build token_mask on CPU to avoid int→device transfers.
            token_mask_cpu = (input_ids == self_inner.config.image_token_id).cpu()
            B, S, H = inputs_embeds.shape
            N = image_features.shape[0]
            src_idx_f = (
                torch.cumsum(token_mask_cpu.float(), dim=-1) - 1
            ).clamp(0, N - 1)  # [B, S], float32
            range_n = torch.arange(N, dtype=torch.float32)
            one_hot = (src_idx_f.reshape(-1, 1) == range_n).float()  # [B*S, N]
            gathered = (
                torch.mm(one_hot, image_features.float().cpu())
                .to(inputs_embeds.dtype)
                .reshape(B, S, H)
                .to(inputs_embeds.device)
            )
            token_mask_dev = (
                token_mask_cpu.to(inputs_embeds.device).unsqueeze(-1).expand_as(inputs_embeds)
            )
            inputs_embeds = torch.where(token_mask_dev, gathered, inputs_embeds)
