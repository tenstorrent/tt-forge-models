...
def _patch_qwen3vl_for_tt_device(model=None):
    ...
    def _patched_fast_pos(self, grid_thw):
        ...
        if cpu_weight is None:
            # Fallback: model was not passed to _patch_qwen3vl_for_tt_device;
            # try eager copy (works if called before model is on TT device).
...
class ModelVariant(StrEnum):
    """Available EgoActor 4B Qwen3VL i1 GGUF model variants for image to text."""

    EGOACTOR_4B_QWEN3VL_I1_GGUF = "4b_qwen3vl_i1_gguf"


class ModelLoader(ForgeModel):
    """EgoActor 4B Qwen3VL i1 GGUF model loader implementation for image to text tasks."""
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        ...
        return ModelInfo(
            model="EgoActor 4B Qwen3VL i1 GGUF",
            variant=variant,
