...
def _register_qwen3vl_gguf_architecture():
    ...
    The noctrex Huihui GGUF stores general.architecture = "qwen3vl" (no
...
def _patch_qwen3vl_for_tt_device(model=None):
    ...
    model is moved to TT (by passing model at call time), then reimplement the
    function body entirely on CPU.
    ...
    def _patched_fast_pos(self, grid_thw):
        # Reimplement fast_pos_embed_interpolate entirely on CPU using the
        # pre-captured pos_embed weight. Do NOT call .to(TT_device) here —
        # that triggers another TT compilation. Instead return the CPU tensor;
        # the resumed Dynamo graph handles host→device transfer implicitly.
