...
def _patch_grouped_mm_experts_forward():
    ...
    CPU (where XLA ops fall back) only supports float input. When device.type is
    "xla" the int path is chosen, causing:
    ...
    def _patched_gmef(
        ...
        # Fix: CUDA supports int histc; CPU and XLA (which falls back to CPU) require float.
    ...
    # Replace both the module-level name and the ExpertsInterface dict entry.
    # ALL_EXPERTS_FUNCTIONS.get_interface("grouped_mm") reads from _global_mapping,
    # so we must update that class attribute to make the patch take effect at runtime.
...
# Register architecture tables at import time (needed for test collection config reads)
