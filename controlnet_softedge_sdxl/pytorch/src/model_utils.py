...
"""
Helper functions for ControlNet Softedge SDXL model loading and processing.
"""
...
def load_controlnet_softedge_sdxl_pipe(controlnet_model_name, base_model_name):
    """Load ControlNet Softedge SDXL pipeline.
...
def create_softedge_conditioning_image(height=1024, width=1024):
    """Create a dummy softedge conditioning image.
    ...
    Returns:
        PIL.Image: A dummy softedge conditioning image
    """
    return Image.new("RGB", (width, height), color=(0, 0, 0))


def controlnet_softedge_sdxl_preprocessing(
    pipe,
    prompt,
    control_image,
    device="cpu",
    ...
    """Preprocess inputs for ControlNet Softedge SDXL model.

    Args:
        pipe: StableDiffusionXLControlNetPipeline
        prompt: Text prompt for generation
        control_image: Softedge conditioning image
