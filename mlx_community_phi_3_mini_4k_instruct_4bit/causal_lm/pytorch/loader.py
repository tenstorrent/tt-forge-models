...
class ModelVariant(StrEnum):
    """Available mlx-community Phi-3-mini-4k-instruct-4bit model variants."""

    PHI_3_MINI_4K_INSTRUCT_4BIT = "Mini_4K_Instruct_4bit"
...
class ModelLoader(ForgeModel):
    ...
    DEFAULT_VARIANT = ModelVariant.PHI_3_MINI_4K_INSTRUCT_4BIT
    ...
    def get_mesh_config(self, num_devices: int):
        ...
        if num_devices == 32:
            mesh_shape = (8, 4)
        elif text_config.num_attention_heads % num_devices == 0:
            mesh_shape = (1, num_devices)
        elif (
            text_config.num_attention_heads % (num_devices // 2) == 0
            and num_devices % 2 == 0
        ):
            mesh_shape = (2, num_devices // 2)
        else:
            raise ValueError(
                f"Cannot evenly distribute {text_config.num_attention_heads} heads across {num_devices} devices"
            )
