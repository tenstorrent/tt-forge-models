...
class ModelLoader(ForgeModel):
    ...
    def get_mesh_config(self, num_devices: int):
        ...
        OLMo-2-32B has 40 attention heads and 8 KV heads; both divide common
        device counts (2, 4, 8), so a (1, N) mesh shards cleanly.
        """
    ...
    def load_shard_spec(self, model):
        """Megatron column->row tensor-parallel shard spec for OLMo-2.

        Attention q/k/v projections are column-sharded on the "model" axis and
        o_proj is row-sharded; the MLP gate/up are column-sharded and down is
        row-sharded. The lm_head is column-sharded (untied embeddings).
        """
        shard_specs = {}
