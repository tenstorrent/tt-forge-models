...
def _patch_transformers_qwen3vlmoe_gguf():
    ...
    We bridge the gap by registering qwen3vlmoe config/tensor mappings and
    converting the model_type to qwen3_moe so Qwen3MoeForCausalLM can load it
    as a text-only causal LM.
    """
    ...
    def patched_load_gguf_checkpoint(*args, **kwargs):
        ...
        if result.get("config", {}).get("model_type") == "qwen3vlmoe":
            ...
            result["config"]["_gguf_source_arch"] = "qwen3vlmoe"
        return result
    ...
    def patched_get_gguf_hf_weights_map(
        ...
        if getattr(hf_model.config, "_gguf_source_arch", None) == "qwen3vlmoe":
