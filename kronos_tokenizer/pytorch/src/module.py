...
#
# Model architecture components for Kronos-Tokenizer.
...
class BinarySphericalQuantizer(nn.Module):
    ...
    def forward(self, z, collect_metrics=True):
        ...
        if self.soft_entropy:
            ...
            zb_by_sample = (
                ((zq + 1) / 2)
                .reshape(z.shape[0], -1, z.shape[-1])
                .to(torch.float32)
            )
            persample_entropy = self.get_hard_per_sample_entropy(zb_by_sample)
            cb_entropy = self._codebook_entropy(zq)
            entropy_penalty = self.gamma0 * persample_entropy - self.gamma * cb_entropy
    ...
    def soft_entropy_loss(self, z):
        ...
        if self.persample_entropy_compute == "analytical":
            if self.l2_norm:
                p = torch.sigmoid(
                    -4 * z / (self.embed_dim**0.5) * self.inv_temperature
                )
            else:
                p = torch.sigmoid(-4 * z * self.inv_temperature)
            prob_stack = torch.stack([p, 1 - p], dim=-1)
            per_sample_entropy = (
                self.get_entropy(prob_stack, dim=-1, normalize=False)
                .sum(dim=-1)
                .mean()
            )
        else:
