...
from third_party.tt_forge_models.vadv2.object_detection.pytorch.src.transformer import *
from third_party.tt_forge_models.vadv2.object_detection.pytorch.src.bbox import (
    CustomNMSFreeCoder,
    MapNMSFreeCoder,
)
import copy
from third_party.tt_forge_models.vadv2.object_detection.pytorch.src.vad_utils import *
...
class VADHead(nn.Module):
        ...
        if self.motion_decoder:
            ...
            if self.motion_map_decoder is not None:
                ...
                if self.use_pe:
                    ...
                    motion_pos = torch.zeros((num_query, batch, 2))
    ...
    def select_and_pad_pred_map(
        ...
        for i in range(map_score.shape[0]):
            ...
            padding_mask = torch.tensor([False]).repeat(batch_max_pnum)
            if pad_pnum != 0:
                valid_map_query = torch.cat(
                    [
                        valid_map_query,
                        torch.zeros((pad_pnum, dim)),
                    ],
                    dim=0,
                )
        ...
        if use_fix_pad:
            pad_map_query = torch.zeros((num_batch, 1, feat_dim))
            pad_map_pos = torch.ones((num_batch, 1, 2))
            pad_lane_mask = torch.tensor([False]).unsqueeze(0).repeat(num_batch, 1)
    ...
    def select_and_pad_query(
        ...
        for i in range(query_score.shape[0]):
            ...
            padding_mask = torch.tensor([False]).repeat(batch_max_qnum)
            if pad_qnum != 0:
                valid_query = torch.cat(
                    [
                        valid_query,
                        torch.zeros((pad_qnum, dim)),
                    ],
                    dim=0,
                )
        ...
        if use_fix_pad:
            ...
            pad_mask = torch.tensor([False]).unsqueeze(0).repeat(num_batch, 1)
