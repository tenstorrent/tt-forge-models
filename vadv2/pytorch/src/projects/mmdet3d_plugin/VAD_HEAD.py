...
from .vad_transformer import CustomTransformerDecoder, VADPerceptionTransformer
from .vad_utils import *
from .custom_bbox_coder import CustomNMSFreeCoder, MapNMSFreeCoder
from ...build_model import HEADS
...
class VADHead(DETRHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bbox_coder = CustomNMSFreeCoder(
            ...
            post_center_range=bbox_coder.get("post_center_range", None),
            max_num=bbox_coder.get("max_num", 100),
        ...
        self.map_bbox_coder = MapNMSFreeCoder(
            pc_range=map_bbox_coder["pc_range"],
            post_center_range=map_bbox_coder.get("post_center_range", None),
            max_num=map_bbox_coder.get("max_num", min(100, map_num_query)),
            score_threshold=map_bbox_coder.get("score_threshold", None),
            num_classes=map_bbox_coder.get("map_num_classes", 3),
        )
        ...
        if kwargs["train_cfg"] is not None:
            assert "map_assigner" in kwargs["train_cfg"], (
                "map assigner should be provided " "when train_cfg is set."
            )
            ...
            assert loss_map_bbox["loss_weight"] == map_assigner["reg_cost"]["weight"], (
        ...
        self.loss_traj = L1Loss(0.2)
        self.loss_traj_cls = FocalLoss(True, 2.0, 0.25, 0.2)
        self.loss_map_bbox = L1Loss(0.0)
        self.loss_map_cls = FocalLoss(True, 2.0, 0.25, 0.8)
        self.loss_map_iou = GIoULoss(0.0)
        self.loss_map_pts = PtsL1Loss(0.4)
        self.loss_map_dir = PtsDirCosLoss(0.005)
        self.loss_plan_reg = L1Loss(1.0)
        self.loss_plan_bound = PlanMapBoundLoss(1.0, 1.0)
        self.loss_plan_col = PlanCollisionLoss(1.0)
        self.loss_plan_dir = PlanMapDirectionLoss(0.5)
    ...
    def _init_layers(self):
        ...
        if not self.as_two_stage:
            ...
            if self.map_query_embed_type == "all_pts":
                self.map_query_embedding = nn.Embedding(
                    self.map_num_query, self.embed_dims * 2
                )
        ...
        if self.motion_decoder is not None:
            self.motion_decoder = CustomTransformerDecoder()
        ...
        if self.motion_map_decoder is not None:
            ...
            self.motion_map_decoder = CustomTransformerDecoder()
            if self.use_pe:
        ...
        if self.ego_agent_decoder is not None:
            self.ego_agent_decoder = CustomTransformerDecoder()
            if self.use_pe:
                self.ego_agent_pos_mlp = nn.Linear(2, self.embed_dims)

        if self.ego_map_decoder is not None:
            self.ego_map_decoder = CustomTransformerDecoder()
            if self.use_pe:
    ...
    def init_weights(self):
        ...
        if self.motion_decoder is not None:
            ...
            if self.use_pe:
                xavier_init(self.pos_mlp_sa, distribution="uniform", bias=0.0)
        if self.motion_map_decoder is not None:
            ...
            for p in self.lane_encoder.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
            if self.use_pe:
                xavier_init(self.pos_mlp, distribution="uniform", bias=0.0)
        if self.ego_his_encoder is not None:
    ...
    def forward(
        ...
        if self.map_query_embed_type == "all_pts":
            ...
            map_query_embeds = (
                (map_pts_embeds + map_instance_embeds).flatten(0, 1).to(dtype)
            )
        ...
        if (
            ...
            outputs = self.transformer(
                ...
                map_reg_branches=(
                    self.map_reg_branches if self.with_box_refine else None
                ),  # noqa:E501
        ...
        if self.motion_decoder is not None:
            ...
            if self.use_pe:
                ...
                motion_pos = (
                    motion_pos.unsqueeze(2).repeat(1, 1, self.fut_mode, 1).flatten(1, 2)
                )
            ...
            if self.motion_det_score is not None:
                ...
                invalid_motion_idx = (
                    invalid_motion_idx.unsqueeze(2)
                    .repeat(1, 1, self.fut_mode)
                    .flatten(1, 2)
                )
            else:
            ...
            if self.motion_map_decoder is not None:
                ...
                motion_coords = outputs_coords_bev[-1]  # [B, A, 2]
                motion_coords = (
                    motion_coords.unsqueeze(2)
                    .repeat(1, 1, self.fut_mode, 1)
                    .flatten(1, 2)
                )
                map_query = map_hs[-1].view(
                    batch_size, self.map_num_vec, self.map_num_pts_per_vec, -1
                )
                ...
                if self.use_pe:
                    ...
                    motion_pos = torch.zeros(
                        (num_query, batch, 2), device=motion_hs.device
                    )
            ...
            motion_hs = torch.cat(
                [motion_hs, ca_motion_query], dim=-1
            )  # [B, A, fut_mode, 2D]
        else:
            raise NotImplementedError("Not implement yet")
        ...
        min_map_pos = min_map_pos[
            range(min_map_pos.shape[0]), min_map_pos_idx
        ]  # [B*P, 2]
        ...
        if self.ego_his_encoder is not None and self.ego_lcf_feat_idx is not None:
            ...
            ego_feats = torch.cat(
                [ego_his_feats, ego_map_query.permute(1, 0, 2)], dim=-1
            )  # [B, 1, 2D]
        ...
        outs = {
            ...
            "all_traj_cls_scores": outputs_trajs_classes.repeat(
                outputs_coords.shape[0], 1, 1, 1
            ),
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        assert len(det_preds_dicts) == len(
            map_preds_dicts
    ...
    def select_and_pad_pred_map(
        ...
        for i in range(map_score.shape[0]):
            ...
            padding_mask = torch.tensor([False], device=map_score.device).repeat(
                batch_max_pnum
            )
            if pad_pnum != 0:
                valid_map_query = torch.cat(
                    [
                        valid_map_query,
                        torch.zeros((pad_pnum, dim), device=map_score.device),
                    ],
                    dim=0,
                )
                valid_map_pos = torch.cat(
                    [
                        valid_map_pos,
                        torch.zeros((pad_pnum, 2), device=map_score.device),
                    ],
                    dim=0,
                )
        ...
        selected_map_query = selected_map_query.unsqueeze(1).repeat(
            1, num_agent, 1, 1
        )  # [B, A, max_P, D]
        selected_map_pos = selected_map_pos.unsqueeze(1).repeat(
            1, num_agent, 1, 1
        )  # [B, A, max_P, 2]
        selected_padding_mask = selected_padding_mask.unsqueeze(1).repeat(
            1, num_agent, 1
        )  # [B, A, max_P]
        # move lane to per-car coords system
        selected_map_dist = (
            selected_map_pos - motion_pos[:, :, None, :]
        )  # [B, A, max_P, 2]
        if pe_normalization:
            selected_map_pos = (
                selected_map_pos - motion_pos[:, :, None, :]
        ...
        map_dis = torch.sqrt(
            selected_map_dist[..., 0] ** 2 + selected_map_dist[..., 1] ** 2
        )
        ...
        if use_fix_pad:
            pad_map_query = torch.zeros(
                (num_batch, 1, feat_dim), device=selected_map_query.device
            )
            pad_map_pos = torch.ones((num_batch, 1, 2), device=selected_map_pos.device)
            pad_lane_mask = (
                torch.tensor([False], device=selected_padding_mask.device)
                .unsqueeze(0)
                .repeat(num_batch, 1)
            )
            ...
            selected_padding_mask = torch.cat(
                [selected_padding_mask, pad_lane_mask], dim=1
            )

        return selected_map_query, selected_map_pos, selected_padding_mask

    def select_and_pad_query(
        self, query, query_pos, query_score, score_thresh=0.5, use_fix_pad=True
    ):
        ...
        for i in range(query_score.shape[0]):
            ...
            padding_mask = torch.tensor([False], device=query_score.device).repeat(
                batch_max_qnum
            )
            if pad_qnum != 0:
                valid_query = torch.cat(
                    [
                        valid_query,
                        torch.zeros((pad_qnum, dim), device=query_score.device),
                    ],
                    dim=0,
                )
                valid_query_pos = torch.cat(
                    [
                        valid_query_pos,
                        torch.zeros((pad_qnum, 2), device=query_score.device),
                    ],
                    dim=0,
                )
        ...
        if use_fix_pad:
            pad_query = torch.zeros(
                (num_batch, 1, feat_dim), device=selected_query.device
            )
            pad_query_pos = torch.ones(
                (num_batch, 1, 2), device=selected_query_pos.device
            )
            pad_mask = (
                torch.tensor([False], device=selected_padding_mask.device)
                .unsqueeze(0)
