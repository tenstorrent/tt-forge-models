...
class VAD(MVXTwoStageDetector):
    ...
    def forward_test(
        ...
        **kwargs,
        ...
        # # Keep test-time API compatible with both:
        # # - img as Tensor [bs, num_cam, C, H, W]
        # # - img as list (multi-augmentation style)
        # # Without this normalization, passing a raw Tensor makes img[0] drop
        # # batch dim and swaps semantics (bs<->num_cam) downstream.
        # if img is None:
        #     img = [img]
    ...
    def map_pred2result(self, bboxes, scores, labels, pts, attrs=None):
        ...
        result_dict = dict(
            map_boxes_3d=bboxes,
            map_scores_3d=scores,
            map_labels_3d=labels,
            map_pts_3d=pts,
        )

        if attrs is not None:
            result_dict["map_attrs_3d"] = attrs
