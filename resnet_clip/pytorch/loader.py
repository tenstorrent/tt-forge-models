...
"""
ResNet CLIP model loader implementation for image-text similarity using OpenCLIP.

OpenAI's original CLIP release includes ResNet-based vision towers (e.g. RN50,
RN101) paired with a transformer text encoder. These checkpoints are hosted
under the `timm/` namespace on HuggingFace and loaded via OpenCLIP.
"""
import torch
...
class ModelVariant(StrEnum):
    """Available ResNet CLIP model variants."""

    RESNET101_OPENAI = "ResNet101_OpenAI"


class ModelLoader(ForgeModel):
    """ResNet CLIP model loader using OpenCLIP for image-text similarity tasks."""

    _VARIANTS = {
        ModelVariant.RESNET101_OPENAI: ModelConfig(
            pretrained_model_name="hf-hub:timm/resnet101_clip.openai",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.RESNET101_OPENAI
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the ResNet CLIP model instance.

        Args:
            dtype_override: Optional torch.dtype to override the model's default dtype.

        Returns:
            torch.nn.Module: The ResNet CLIP model instance.
        """
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the ResNet CLIP model.
