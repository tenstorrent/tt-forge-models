...
"""
Helper functions for ControlNet QR Code Monster SDXL model loading and processing.
"""
...
def load_controlnet_qrcode_monster_sdxl_pipe(controlnet_model_name, base_model_name):
    """Load ControlNet QR Code Monster SDXL pipeline.
...
def create_qrcode_conditioning_image(height=1024, width=1024, module_size=16):
    """Create a dummy QR code conditioning image.

    Generates a synthetic black-and-white QR-like pattern with the recommended
    16px module size for the QR Code Monster ControlNet.
    ...
    Returns:
        PIL.Image: A dummy QR code conditioning image
    """
    modules_h = height // module_size
    modules_w = width // module_size
    rng = np.random.default_rng(seed=0)
    pattern = rng.integers(0, 2, size=(modules_h, modules_w), dtype=np.uint8) * 255
    image = np.repeat(np.repeat(pattern, module_size, axis=0), module_size, axis=1)
    image = np.stack([image] * 3, axis=-1)
    return Image.fromarray(image)


def controlnet_qrcode_monster_sdxl_preprocessing(
    pipe,
    prompt,
    control_image,
    device="cpu",
    ...
    """Preprocess inputs for ControlNet QR Code Monster SDXL model.

    Args:
        pipe: StableDiffusionXLControlNetPipeline
        prompt: Text prompt for generation
        control_image: QR code conditioning image
