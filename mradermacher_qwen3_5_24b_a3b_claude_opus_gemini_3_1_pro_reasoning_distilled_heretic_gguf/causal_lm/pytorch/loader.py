...
def _patch_transformers_qwen35moe_gguf():
    ...
    for the qwen35moe architecture. Also re-patches load_gguf_checkpoint with
    *args/**kwargs to fix session contamination from narrow-sig loaders.
    Called at both module level and inside load_model() for robustness.
    """
    ...
    # 1. Register qwen35moe (idempotent, no early return so we re-patch every call)
    ...
    def patched_load_gguf_checkpoint(*args, **kwargs):
        ...
        # Fix depthwise Conv1d weight shape: GGUF stores as (C, K) but
        # nn.Conv1d(groups=C) expects (C, 1, K). Unsqueeze dim 1.
        if result.get("tensors"):
            import torch
