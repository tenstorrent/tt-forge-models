...
"""
Kronk3.5-9B GGUF model loader implementation for causal language modeling.

Kronk3.5-9B is a Qwen3.5-based hybrid SSM/full-attention model (GGUF architecture
"qwen35") that alternates GatedDeltaNet linear-attention layers with periodic
full-attention layers every 4 blocks.  Transformers 5.2 ships Qwen3_5ForCausalLM
which models this architecture, but the GGUF loader has no config-field or
tensor-name mapping for "qwen35".  This loader monkey-patches that gap via a
context manager so other GGUF loaders that patch the same binding sites do not
interfere at run time.
"""
...
class _Qwen35TensorProcessor(TensorProcessor):
    ...
    def process(self, weights: np.ndarray, name: str, **kwargs) -> GGUFTensor:
        if "ssm_a" in name and ".weight" not in name:
            # GGUF stores ssm_a as raw positive values; convert to A_log = log(-ssm_a).
...
class ModelVariant(StrEnum):
    """Available Kronk3.5-9B GGUF model variants for causal language modeling."""

    KRONK3_5_9B_GGUF = "9B_GGUF"


class ModelLoader(ForgeModel):
    """Kronk3.5-9B GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.KRONK3_5_9B_GGUF: LLMModelConfig(
            pretrained_model_name="mradermacher/Kronk3.5-9B-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.KRONK3_5_9B_GGUF

    GGUF_FILE = "Kronk3.5-9B.Q4_K_M.gguf"
