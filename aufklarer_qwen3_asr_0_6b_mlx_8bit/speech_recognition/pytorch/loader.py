...
"""
aufklarer/Qwen3-ASR-0.6B-MLX-8bit model loader implementation for speech recognition (ASR).

aufklarer/Qwen3-ASR-0.6B-MLX-8bit is an 8-bit MLX-quantized conversion of
Qwen/Qwen3-ASR-0.6B, a multilingual automatic speech recognition model from
Alibaba's Qwen team.
"""
...
class ModelVariant(StrEnum):
    """Available aufklarer/Qwen3-ASR-0.6B-MLX-8bit speech recognition model variants."""

    V0_6B_MLX_8BIT = "0.6B_MLX_8bit"
...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.V0_6B_MLX_8BIT: ModelConfig(
            pretrained_model_name="aufklarer/Qwen3-ASR-0.6B-MLX-8bit",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.V0_6B_MLX_8BIT
    ...
    def _load_model_wrapper(self, dtype_override=None):
        ...
        # The MLX quantization config ({"group_size", "bits", "mode"}) lacks
        # the `quant_method` field that transformers expects, causing a
        # ValueError. Remove it so the model loads as a standard float model.
    ...
    def load_inputs(self, dtype_override=None):
        ...
        model_dtype = next(self._model_wrapper.model.thinker.parameters()).dtype

        return [
            inputs["input_ids"],
            inputs["attention_mask"],
            inputs["input_features"].to(model_dtype),
