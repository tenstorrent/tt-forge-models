...
class TwoStageDetector(BaseDetector):
    ...
    def reorder_first_stage_pred_and_feature(self, first_pred, example, features):
        ...
        rois = first_pred[0]["box3d_lidar"].new_zeros(
            (batch_size, self.NMS_POST_MAXSIZE, box_length)
        )
        roi_scores = first_pred[0]["scores"].new_zeros(
            (batch_size, self.NMS_POST_MAXSIZE)
        )
        roi_labels = first_pred[0]["label_preds"].new_zeros(
            (batch_size, self.NMS_POST_MAXSIZE), dtype=torch.long
        )
        roi_features = features[0][0].new_zeros(
            (batch_size, self.NMS_POST_MAXSIZE, feature_vector_length)
        )
    ...
    def post_process(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            scores = torch.sqrt(
                torch.sigmoid(cls_preds).reshape(-1)
                * batch_dict["roi_scores"][index].reshape(-1)
            )
    ...
    def forward(self, example, return_loss=True, **kwargs):
        ...
        if self.roi_head.code_size == 7 and return_loss is True:
            # drop velocity
            example["gt_boxes_and_cls"] = example["gt_boxes_and_cls"][
                :, :, [0, 1, 2, 3, 4, 5, 6, -1]
            ]
        ...
        example = self.reorder_first_stage_pred_and_feature(
            first_pred=one_stage_pred, example=example, features=features
        )
