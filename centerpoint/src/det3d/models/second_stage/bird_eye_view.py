...
class BEVFeatureExtractor(nn.Module):
    ...
    def absl_to_relative(self, absolute):
        a1 = (
            (absolute[..., 0] - self.pc_start[0]) / self.voxel_size[0] / self.out_stride
        )
        a2 = (
            (absolute[..., 1] - self.pc_start[1]) / self.voxel_size[1] / self.out_stride
        )
    ...
    def forward(self, example, batch_centers, num_point):
        ...
        for batch_idx in range(batch_size):
            ...
            # N x C
            feature_map = bilinear_interpolate_torch(
                example["bev_feature"][batch_idx], xs, ys
            )
            ...
            if num_point > 1:
                ...
                feature_map = torch.cat(
                    [
                        feature_map[i * section_size : (i + 1) * section_size]
