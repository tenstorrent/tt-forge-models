...
def parse_args():
    ...
    parser.add_argument("--checkpoint", help="the dir to prediction file")
...
def veh_pos_to_transform(veh_pos):
    ...
    global_from_car = transform_matrix(tran, Quaternion(matrix=rotation), inverse=False)

    car_from_global = transform_matrix(tran, Quaternion(matrix=rotation), inverse=True)
...
def main():
    ...
    tracker = Tracker(
        max_age=args.max_age, max_dist=max_dist, score_thresh=args.score_thresh
    )
    ...
    global_preds, detection_results = convert_detection_to_global_box(
        predictions, infos
    )
    ...
    print(
        "waymo-open-dataset/bazel-bin/waymo_open_dataset/metrics/tools/compute_tracking_metrics_main \
           {}  {} ".format(
            result_path, gt_path
        )
    )
...
def transform_box(box, pose):
    ...
    center = np.einsum(
        "...ij,...nj->...ni", transform[..., 0:3, 0:3], box[..., 0:3]
    ) + np.expand_dims(transform[..., 0:3, 3], axis=-2)

    velocity = box[..., [6, 7]]

    velocity = np.concatenate(
        [velocity, np.zeros((velocity.shape[0], 1))], axis=-1
    )  # add z velocity

    velocity = np.einsum("...ij,...nj->...ni", transform[..., 0:3, 0:3], velocity)[
        ..., [0, 1]
    ]  # remove z axis

    return np.concatenate(
        [center, box[..., 3:6], velocity, heading[..., np.newaxis]], axis=-1
    )
