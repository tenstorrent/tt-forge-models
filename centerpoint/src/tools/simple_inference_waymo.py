...
def run_model(points, fp16=False):
    ...
    return {
        "boxes": outputs["box3d_lidar"].cpu().numpy(),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint",
        help="the path to checkpoint which the model read from",
        default=None,
        type=str,
    )
    ...
    print(
        "Please prepare your point cloud in waymo format and save it as a pickle dict with points key into the {}".format(
            args.input_data_dir
        )
    )
