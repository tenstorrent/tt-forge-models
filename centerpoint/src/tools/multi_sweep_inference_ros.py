    ...
    def get_lidar_data(self, input_points: dict):
        ...
        if len(self.lidar_deque) == 5:
            ...
            car_from_global = transform_matrix(
                self.lidar_deque[-1]["translation"],
                self.lidar_deque[-1]["rotation"],
                inverse=True,
            )
            ...
            for i in range(len(self.lidar_deque) - 1):
                ...
                global_from_car = transform_matrix(
                    self.lidar_deque[i]["translation"],
                    self.lidar_deque[i]["rotation"],
                    inverse=False,
                )
                ...
                last_pc_gpu = cp.vstack(
                    (last_pc_gpu[:3, :], cp.ones(last_pc_gpu.shape[1]))
                )
            ...
            for i in range(len(self.pc_list)):
                tmp_pc = cp.vstack(
                    (self.pc_list[i], cp.zeros((2, self.pc_list[i].shape[1])))
                )
            ...
            self.points = all_pc
            sync_cloud = xyz_array_to_pointcloud2(
                all_pc[:, :3],
                stamp=self.lidar_deque[-1]["lidar_stamp"],
                frame_id="lidar_top",
            )
...
def rslidar_callback(msg):
    ...
    input_points = {"stamp": stamp, "seq": seq, "points": np_p}
...
if __name__ == "__main__":
    ...
    sub_odom_topic = ["/golfcar/odom", "/aligned/odometry", "/odom"]
