...
"""
Helper functions for ControlNet MediaPipe Face model loading and processing.
"""
...
def load_controlnet_mediapipe_face_pipe(controlnet_model_name, base_model_name):
    """Load ControlNet MediaPipe Face pipeline.
...
def create_face_conditioning_image(height=512, width=512):
    """Create a dummy MediaPipe face conditioning image.
    ...
    Returns:
        PIL.Image: A dummy face conditioning image
    """
    return Image.new("RGB", (width, height), color=(0, 0, 0))


def controlnet_mediapipe_face_preprocessing(
    pipe,
    prompt,
    control_image,
    device="cpu",
    ...
    """Preprocess inputs for ControlNet MediaPipe Face model.

    Args:
        pipe: StableDiffusionControlNetPipeline
        prompt: Text prompt for generation
        control_image: MediaPipe face conditioning image
