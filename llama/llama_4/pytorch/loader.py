...
"""
Llama 4 multimodal model loader (AutoProcessor + AutoModelForImageTextToText).
"""
...
class ModelLoader(ForgeModel):
    """Llama 4 Scout multimodal loader (image + text)."""
    ...
    DEFAULT_VARIANT = ModelVariant.LLAMA_4_SCOUT_17B_16E
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load Llama 4 with AutoModelForImageTextToText."""
    ...
    def load_shard_spec(self, model):
        ...
        Vision blocks use Linear with bias; text attention uses bias=False.
        MoE layers use Llama4TextExperts Parameters (gpt_oss-style EP+FSDP on
        gate_up_proj [E, H, 2I] / down_proj [E, I, H]). Dense layers use
        Llama4TextMLP (e.g. Maverick interleave / non-moe_layers).
        """
        shard_specs = {}
        ...
        # Llama4MultiModalProjector is a single Linear (no linear_2).
        shard_specs[model.multi_modal_projector.linear_1.weight] = ("model", "batch")

        lm = model.language_model
        text = lm.model
        shard_specs[text.embed_tokens.weight] = (None, "batch")
        shard_specs[text.norm.weight] = ("batch",)
        ...
        for layer in text.layers:
            ...
            if hasattr(ff, "router"):
                # Router sees all experts; replicate expert dim, FSDP on hidden.
                shard_specs[ff.router.weight] = (None, "batch")
                # [E, H, 2I] / [E, I, H] — same layout as gpt_oss.
