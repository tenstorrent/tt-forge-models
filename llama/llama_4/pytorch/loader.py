...
def _materialize_vision_rotary_if_meta(model) -> None:
    """Ensure vision RoPE buffer is real storage, not a meta placeholder.

    Some init / SPMD paths leave ``Llama4VisionRotaryEmbedding.freqs_ci`` on the meta
    device; the first forward then does ``freqs_ci.to(device)`` and tt_torch can raise
    ``NotImplementedError: Cannot copy out of meta tensor; no data!``.
    Recomputing the buffer on CPU matches Transformers' ``_compute_freqs_ci`` logic.
    """
    rot = model.vision_model.rotary_embedding
    buf = rot.freqs_ci
    if buf is None or not getattr(buf, "is_meta", False):
        return
    vcfg = model.config.vision_config
    freqs = type(rot)._compute_freqs_ci(vcfg)
    rot.register_buffer("freqs_ci", freqs, persistent=False)
...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        model.eval()
        _materialize_vision_rotary_if_meta(model)
