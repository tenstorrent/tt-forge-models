...
"""
GPT-oss-sft-s1K i1 GGUF model loader implementation for causal language modeling.
"""
import torch
...
def _patch_gpt_oss_support():
    """Register gpt-oss GGUF architecture as an alias for qwen3_moe.

    GPT-oss-sft-s1K uses the same Qwen3-MoE architecture under the OpenAI
    gpt-oss branding. The GGUF file declares architecture as 'gpt-oss' which
    transformers 5.x does not recognise natively.
    """
...
class ModelVariant(StrEnum):
    """Available GPT-oss-sft-s1K i1 GGUF model variants for causal language modeling."""

    GPT_OSS_SFT_S1K_I1_Q4_K_M_GGUF = "GPT_OSS_SFT_S1K_I1_Q4_K_M_GGUF"


class ModelLoader(ForgeModel):
    """GPT-oss-sft-s1K i1 GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.GPT_OSS_SFT_S1K_I1_Q4_K_M_GGUF: LLMModelConfig(
            pretrained_model_name="mradermacher/GPT-oss-sft-s1K-i1-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.GPT_OSS_SFT_S1K_I1_Q4_K_M_GGUF

    GGUF_FILE = "GPT-oss-sft-s1K.i1-Q4_K_M.gguf"
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        # Qwen3Moe: use batched_mm to avoid histc-on-Int failure on XLA device.
