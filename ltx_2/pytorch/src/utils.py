...
def _patch_squeeze_non_aliasing() -> None:
    ...
    The LTX-2 audio-video cross-attention modulation uses many ``.squeeze(dim)``
    calls on size-1 dims (e.g. ``1 + video_a2v_ca_scale.squeeze(2)`` in
    ``LTX2VideoTransformerBlock.forward``). Under torch_xla's CPU-fallback graph
    partitioner these lower to ``prims::view_of``, whose alias annotation cannot
    be functionalized, aborting compilation before the graph reaches the device.
    Replacing the squeeze with an equivalent ``reshape`` (numerically identical)
    removes the alias annotation. Idempotent and applied process-wide at load.
    """
...
def load_transformer(dtype: torch.dtype):
    ...
    # Avoid the prims::view_of functionalization gap in the cross-modality
    # modulation (.squeeze on size-1 dims) before tracing the denoiser.
