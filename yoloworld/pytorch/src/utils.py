...
class ImgDataPreprocessor(BaseDataPreprocessor):
    def __init__(
        ...
        if mean is not None:
            ...
            self.register_buffer("mean", torch.tensor(mean,dtype=torch.bfloat16).view(-1, 1, 1), False)
            self.register_buffer("std", torch.tensor(std,dtype=torch.bfloat16).view(-1, 1, 1), False)
        else:
