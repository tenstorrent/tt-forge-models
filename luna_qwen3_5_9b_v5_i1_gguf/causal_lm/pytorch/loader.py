...
Luna-Qwen3.5-9B-v5 is a hybrid SSM/full-attention model (GGUF architecture "qwen35")
...
for "qwen35".  This loader monkey-patches that gap via a context manager so other
GGUF loaders that patch the same binding sites do not interfere at run time.
"""
...
def _patch_qwen35_registry() -> None:
    ...
    # Patch get_gguf_hf_weights_map once so qwen3_5_text maps back to qwen35.
...
# Apply the idempotent registry patches at import time so the GGUF arch is
# recognised during tokenizer loading (which precedes model loading).
...
class ModelVariant(StrEnum):
    """Available Luna-Qwen3.5-9B-v5 i1 GGUF model variants for causal language modeling."""

    LUNA_QWEN3_5_9B_V5_I1_GGUF = "9B_v5_i1_GGUF"


class ModelLoader(ForgeModel):
    """Luna-Qwen3.5-9B-v5 i1 GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.LUNA_QWEN3_5_9B_V5_I1_GGUF: LLMModelConfig(
            pretrained_model_name="mradermacher/Luna-Qwen3.5-9B-v5-i1-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.LUNA_QWEN3_5_9B_V5_I1_GGUF

    GGUF_FILE = "Luna-Qwen3.5-9B-v5.i1-Q4_K_M.gguf"
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="Luna-Qwen3.5-9B-v5 i1 GGUF",
            variant=variant,
