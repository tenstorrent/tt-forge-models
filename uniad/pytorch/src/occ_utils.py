...
from third_party.tt_forge_models.uniad.pytorch.src.base_module import BaseModule
    ...
    def __init__(
        ...
        **kwargs
        ...
        if post_norm_cfg is not None:
            self.post_norm = torch.nn.LayerNorm(
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if out_channels == in_channels and not downsample and not upsample:
            ...
            if upsample:
                projection.update({"upsample_skip_proj": F.Interpolate(scale_factor=2)})
            elif downsample:
                projection.update(
