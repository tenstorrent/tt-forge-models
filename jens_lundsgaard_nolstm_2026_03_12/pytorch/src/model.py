...
#
# Vendored from JensLundsgaard/nolstm-2026-03-12 (raffael_model.py) on
# HuggingFace Hub. Adjusted only to fix the sibling import path.
"""
ConvLSTM Autoencoder for spatiotemporal (video-like) reconstruction.
"""
import torch
import torch.nn as nn
import torch.nn.functional as F

from .conv_lstm import ConvLSTM  # noqa: F401 (kept for parity with upstream)
...
class Encoder(nn.Module):
    """2D CNN spatial compression + LSTM temporal modeling -> (B, T, latent_size)."""
    ...
    def forward(self, x):
        ...
        if H != 128 or W != 128:
            x = torch.nn.functional.interpolate(
                x, size=(128, 128), mode="bilinear", align_corners=True
            )
...
class ConvLSTMAutoencoder(nn.Module):
    """Complete ConvLSTM Autoencoder operating on 128x128 image sequences."""
    ...
    def forward(self, x):
        ...
        if orig_H != 128 or orig_W != 128:
            ...
            x_rec_flat = torch.nn.functional.interpolate(
