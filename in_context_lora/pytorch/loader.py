...
"""
In-Context LoRA for FLUX model loader implementation.

Loads the ostris/OpenFLUX.1 base pipeline (open-weight FLUX variant) and applies
task-specific In-Context LoRA weights from ali-vilab/In-Context-LoRA for multi-task
...
class ModelLoader(ForgeModel):
    """In-Context LoRA for FLUX model loader."""
    ...
    def _load_pipeline(self, dtype_override=None):
        """Load OpenFLUX.1 pipeline with In-Context LoRA weights applied."""
    ...
    def load_inputs(self, dtype_override=None, batch_size=1) -> Any:
        ...
        Returns:
            dict with transformer input tensors.
        """
        if self.pipe is None:
            self._load_pipeline(dtype_override=dtype_override)
