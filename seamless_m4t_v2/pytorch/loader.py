...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the SeamlessM4T v2 text decoder submodule for this instance's variant.
    ...
    def load_inputs(self, batch_size=1):
        """Load and return sample inputs for the SeamlessM4T v2 model with this instance's variant settings.
        ...
        # module instead of torchaudio.load so we don't depend on an FFmpeg /
        # torchcodec backend being present in the environment.
        ...
        # Map sample width to the corresponding signed-integer dtype and the
        # full-scale value used to normalize to floating point in [-1, 1].
        dtype_map = {1: np.int8, 2: np.int16, 4: np.int32}
        if sampwidth not in dtype_map:
            raise ValueError(f"Unsupported WAV sample width: {sampwidth} bytes")
        samples = np.frombuffer(frames, dtype=dtype_map[sampwidth]).astype(np.float32)
        samples /= float(np.iinfo(dtype_map[sampwidth]).max)

        # Reshape to (channels, samples) to match torchaudio.load's layout.
        audio = torch.from_numpy(samples).reshape(-1, n_channels).t().contiguous()

        # Resample audio (torchaudio.functional.resample is pure-torch, no codec).
