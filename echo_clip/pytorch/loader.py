...
"""
Echo-CLIP model loader implementation for echocardiogram image-text similarity using OpenCLIP.
"""
import torch
...
class ModelVariant(StrEnum):
    """Available Echo-CLIP model variants."""

    CONVNEXT_BASE = "convnext_base"


class ModelLoader(ForgeModel):
    """Echo-CLIP model loader using OpenCLIP for echocardiogram image-text similarity."""

    _VARIANTS = {
        ModelVariant.CONVNEXT_BASE: ModelConfig(
            pretrained_model_name="hf-hub:mkaichristensen/echo-clip",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.CONVNEXT_BASE
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the Echo-CLIP model instance.

        Args:
            dtype_override: Optional torch.dtype to override the model's default dtype.

        Returns:
            torch.nn.Module: The Echo-CLIP model instance.
        """
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the Echo-CLIP model.
    ...
    def post_process(self, outputs):
        """Post-process Echo-CLIP model outputs to extract similarity scores.
