...
class ModelLoader(ForgeModel):
    ...
    def __init__(
        ...
        Args:
            ...
            mlp_type: MLP type to use. "a2a_sparse" replaces MLP with A2aSparseMLP for
                      dispatch/combine expert parallelism. Any other value (e.g. "sparse")
                      keeps the original MLP module.
        """
        ...
        if os.environ.get("GPT_OSS_MLP_TYPE"):
            self.mlp_type = os.environ["GPT_OSS_MLP_TYPE"]
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        if self.mlp_type == "a2a_sparse":
            for layer in model.model.layers:
                mesh, _ = self.get_mesh_config(64)
                cluster_axis = 0
    ...
    def load_shard_spec(self, model):
        ...
        # "a2a_sparse_device_only": deferred replacement — swap original MLP to
        # A2aSparseMLP now (before device compilation) so CPU golden used original MLP.
        if self.mlp_type == "a2a_sparse_device_only":
            from tt_torch.sparse_mlp import A2aSparseMLP
            for layer in model.model.layers:
                if not isinstance(layer.mlp, A2aSparseMLP):
                    mesh, _ = self.get_mesh_config(8)
                    cluster_axis = 0
                    layer.mlp = A2aSparseMLP(
                        layer.mlp,
                        num_experts=self.config.num_local_experts,
        ...
        # Determine effective mlp_type for shard specs
        effective_mlp_type = "a2a_sparse" if self.mlp_type == "a2a_sparse_device_only" else self.mlp_type
        ...
        for layer in model.model.layers:
            ...
            if effective_mlp_type == "a2a_sparse":
