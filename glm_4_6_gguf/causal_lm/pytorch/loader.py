...
def _patch_transformers_glm4moe_gguf():
    ...
    for the glm4moe architecture. GLM-4.6 is a MoE model that uses the
    glm4moe architecture identifier in its GGUF metadata.
    """
    ...
    def patched_load_gguf_checkpoint(*args, **kwargs):
        ...
        if config.get("model_type") == "glm4moe":
            ...
            # Compute partial_rotary_factor from rope.dimension_count / attention.key_length
            if hasattr(args[0] if args else None, "__fspath__") or isinstance(
                ...
                try:
                    ...
                    for key, field in reader.fields.items():
                        ...
                        elif "attention.key_length" in key:
                            key_len = _gguf_parse_value(
                                field.parts[field.data[0]], field.types
                            )
                    if rope_dim is not None and key_len is not None and key_len > 0:
                        config["partial_rotary_factor"] = rope_dim / key_len
                except Exception:
                    pass
        return result
    ...
    # 5. Patch get_gguf_hf_weights_map to remap glm4_moe -> glm4moe for gguf-py.
    # gguf-py uses "glm4moe" (no underscore) but transformers model_type is "glm4_moe".
