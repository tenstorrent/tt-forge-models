...
"""
GLM-4.7-Flash GGUF (ggml-org) model loader implementation for causal language modeling.
"""
import torch
...
def _patch_transformers_deepseek2_gguf():
    ...
    # load_gguf_checkpoint renames model_type deepseek2 -> deepseek_v2, so register both
...
class ModelVariant(StrEnum):
    """Available GLM-4.7-Flash GGUF (ggml-org) model variants for causal language modeling."""

    GLM_4_7_FLASH_GGML_ORG_Q4_K = "4.7_Flash_ggml_org_Q4_K"


class ModelLoader(ForgeModel):
    """GLM-4.7-Flash GGUF (ggml-org) model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.GLM_4_7_FLASH_GGML_ORG_Q4_K: LLMModelConfig(
            pretrained_model_name="ggml-org/GLM-4.7-Flash-GGUF",
            max_length=128,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.GLM_4_7_FLASH_GGML_ORG_Q4_K

    GGUF_FILE = "GLM-4.7-Flash-Q4_K.gguf"
    ...
    def _get_model_info(cls, variant: Optional[ModelVariant] = None) -> ModelInfo:
        return ModelInfo(
            model="GLM-4.7-Flash GGUF (ggml-org)",
            variant=variant,
