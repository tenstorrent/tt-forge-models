...
class ModelVariant(StrEnum):
    """Available GroupViT model variants for image-text similarity."""

    GCC_YFCC = "Gcc_Yfcc"


class ModelLoader(ForgeModel):
    """GroupViT model loader implementation for image-text similarity tasks."""

    # Dictionary of available model variants using structured configs
    _VARIANTS = {
        ModelVariant.GCC_YFCC: ModelConfig(
            pretrained_model_name="nvidia/groupvit-gcc-yfcc",
        ),
    }

    # Default variant to use
    DEFAULT_VARIANT = ModelVariant.GCC_YFCC
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        """Load and return the GroupViT model instance for this instance's variant.
        ...
        Returns:
            torch.nn.Module: The GroupViT model instance for image-text similarity.
        """
        ...
        model = GroupViTModel.from_pretrained(pretrained_model_name, **model_kwargs)
        model.eval()

        return model

    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the GroupViT model with this instance's variant settings.
    ...
    def post_process(self, outputs):
        """Post-process GroupViT model outputs to extract similarity scores.
    ...
    def unpack_forward_output(self, fwd_output):
        ...
        The GroupViT model (with return_dict=False) returns a tuple containing
        similarity logits and embeddings. For training, we concatenate the main
        output tensors to create a single differentiable tensor for backpropagation.
