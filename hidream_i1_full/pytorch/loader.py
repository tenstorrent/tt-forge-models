...
HiDream-I1-Full is the base (non-distilled) checkpoint of the HiDream-I1
family. It shares its architecture exactly with HiDream-I1-Fast — the same
six independently-loadable components — and differs only in the checkpoint
weights and the inference recipe (Full: UniPCMultistepScheduler, 50 steps,
guidance_scale=5.0; Fast: distilled FlashFlowMatchEulerDiscrete, 16 steps,
guidance_scale=0.0).

Because the component architectures are identical, this loader reuses the
wrappers, component loaders and shard specs from ``hidream_i1`` and only
re-points the HiDream repo id at ``HiDream-ai/HiDream-I1-Full``.
...
# Reuse the HiDream-I1 component loaders, wrappers, shard specs and shape
# constants verbatim — the Full and Fast checkpoints are architecturally
# identical, so only the HiDream repo id changes.
from ...hidream_i1.pytorch.src.model_utils import (
    CLIP_VOCAB_SIZE,
    DTYPE,
    LATENT_CHANNELS,
    LATENT_H,
...
# HiDream-I1-Full base checkpoint (vs. HiDream-ai/HiDream-I1-Fast).
...
class ModelLoader(ForgeModel):
    ...
    separately from the Meta repo (the HiDream snapshot does not ship Llama).
    """
