...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.MOLMO_72B_0924: LLMModelConfig(
            ...
            # Image crops insert many patch tokens; Molmo's processor default is 1536.
            max_length=1536,
        ),
    }
    ...
    def _load_processor(self, dtype_override=None):
        ...
        # Keep tokenizer attribute in sync with the processor's tokenizer.
    ...
    def load_inputs(
        ...
        """Load image+text sample inputs for Molmo.

        Uses ``MolmoProcessor.process`` so the model receives the multimodal
        tensors it expects: ``input_ids``, ``images``, ``image_masks``, and
        ``image_input_idx``.

        Args:
            dtype_override: Optional torch.dtype for floating-point image tensors.
            batch_size: Batch size for the inputs.
            prompt: Optional text prompt; defaults to ``image_prompt``.
        ...
        text = prompt or self.image_prompt
        sequence_length = self._variant_config.max_length

        # MolmoProcessor.process (not __call__) builds image crops + patch maps.
        ...
        # MolmoProcessor returns unbatched tensors; match the HF demo and add
        # a leading batch dim, then expand to the requested batch_size.
        inputs = {}
        for key, value in batch.items():
            ...
            if batch_size > 1:
                value = value.repeat_interleave(batch_size, dim=0)
            # Processor emits image_input_idx as int32; torch.arange batch
            # indices are int64. Mixed S32/S64 indexing fails under compile/XLA
            # (Cannot concatenate arrays with different element types).
            if key == "image_input_idx":
                value = value.to(torch.long)
            elif dtype_override is not None and value.is_floating_point():
                value = cast_input_to_type(value, dtype_override)
            inputs[key] = value
