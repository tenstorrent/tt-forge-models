...
# Import flash_attn's attention if available; otherwise fall back to dense
# torch SDPA implementations. flash_attn is a CUDA-only dependency and is not
# present in the Tenstorrent bringup environment. The Infinity inference path
# uses customized_flash_attn=False/use_flex_attn=False, so self-attention never
# calls flash_attn_func; only the text cross-attention uses the varlen kvpacked
# kernel, which the dense fallback below replicates exactly for batch size 1
# (the bringup batch) and per-segment for larger batches (golden/CPU runs).
try:
    ...
    def flash_attn_func(q, k, v, dropout_p=0.0, softmax_scale=None, **kw):
        # q, k, v: (B, L, H, c); ret: (B, L, H, c)
        qh = q.transpose(1, 2)
        kh = k.transpose(1, 2)
        vh = v.transpose(1, 2)
        o = slow_attn(qh, kh, vh, scale=softmax_scale, dropout_p=0.0)
        return o.transpose(1, 2)

    def flash_attn_varlen_kvpacked_func(
        q, kv, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
        dropout_p=0.0, softmax_scale=None, **kw,
    ):
        # q: (Tq, H, c); kv: (Tk, 2, H, c); ret: (Tq, H, c)
        k = kv[:, 0]
        v = kv[:, 1]
        nseg = cu_seqlens_q.numel() - 1
        if nseg == 1:
            qh = q.transpose(0, 1).unsqueeze(0)   # (1, H, Tq, c)
            kh = k.transpose(0, 1).unsqueeze(0)
            vh = v.transpose(0, 1).unsqueeze(0)
            o = slow_attn(qh, kh, vh, scale=softmax_scale, dropout_p=0.0)
            return o.squeeze(0).transpose(0, 1)   # (Tq, H, c)
        ...
        for i in range(nseg):
            ...
            o = slow_attn(qi, ki, vi, scale=softmax_scale, dropout_p=0.0)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        ...
        # qkv: amp, bf16
        qkv = F.linear(x, self.mat_qkv.weight, torch.cat((self.q_bias, self.zero_k_bias, self.v_bias))).view(B, L, 3, self.num_heads, self.head_dim)  # BL3Hc
        ...
        # TT bringup: cos_attn normalizes q/k in fp32 while v stays bf16; align
        # q/k to v's dtype so the single-dtype device graph stays consistent
        # (the flash path below already does this cast explicitly).
        q = q.to(v.dtype)
        k = k.to(v.dtype)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        kv_compact = F.linear(kv_compact, self.mat_kv.weight, torch.cat((self.zero_k_bias, self.v_bias))).view(N, 2, self.num_heads, self.head_dim) # NC => N2Hc
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            x = x + self.ca(self.ca_norm(x), ca_kv).mul(self.ca_gamma)  # TT bringup: drop fp32 upcast
