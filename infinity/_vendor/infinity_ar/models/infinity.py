...
import infinity_ar.utils.dist as dist
from infinity_ar.utils.dist import for_visualize
from infinity_ar.models.basic import flash_attn_func, flash_fused_op_installed, AdaLNBeforeHead, CrossAttnBlock, SelfAttnBlock, CrossAttention, FastRMSNorm, precompute_rope2d_freqs_grid
from infinity_ar.utils import misc
from infinity_ar.models.flex_attn import FlexAttn
from infinity_ar.utils.dynamic_resolution import dynamic_resolution_h_w, h_div_w_templates

try:
    from infinity_ar.models.fused_op import fused_ada_layer_norm, fused_ada_rms_norm
except:
...
class Infinity(nn.Module):
    ...
    def get_logits(self, h: torch.Tensor, cond_BD: Optional[torch.Tensor]):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            # TT bringup: keep a single (bf16) dtype through the head instead of
            # the upstream fp32 upcast, which clashes with bf16 device weights.
    ...
    def add_lvl_embeding(self, feature, scale_ind, scale_schedule, need_to_pad=0):
        ...
        # TT bringup: the upstream in-place slice scatter-add
        #   feature[:, :N] += lvl_embed(...)
        # crashes the tt-mlir compiler. With need_to_pad == 0 (single forward),
        # N == seq_len, so this is a plain full-tensor add of the per-level
        # embedding (a single learned vector broadcast over the sequence).
        lvl_vec = self.lvl_embed.weight[scale_ind].to(feature.dtype)  # (c,)
        return feature + lvl_vec
    ...
    def add_lvl_embeding_for_x_BLC(self, x_BLC, scale_schedule, need_to_pad=0):
        ...
        for scale_ind, patch_t_h_w in enumerate(scale_schedule):
            # TT bringup: keep this a Python int (was np.int64), so the slice
            # index / pointer arithmetic stays traceable under torch.compile.
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        # TT bringup: keep the input in its incoming (bf16) dtype rather than the
        # upstream fp32 upcast, so the whole graph runs in one dtype on device.
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            total = 0
            for le in lens:
                # TT bringup: skip the RNG call entirely when cond-drop is off
                # (inference) — random.random() is a graph break under Dynamo.
                if self.cond_drop_rate > 0 and random.random() < self.cond_drop_rate:
            ...
            sos = cond_BD = self.text_proj_for_sos((kv_compact, cu_seqlens_k, max_seqlen_k)).contiguous()    # TT bringup: drop fp32 upcast
