...
"""
OLMo 3.1 32B Think GGUF model loader implementation for causal language modeling.
"""
import torch
...
def _patch_olmo2_support():
    """Register olmo2 architecture for GGUF loading.

    OLMo2 uses the same general structure as LLaMA but the GGUF file declares
    architecture as 'olmo2' which transformers does not recognise for GGUF.
    """
    ...
    for section in _gguf_utils.GGUF_TO_TRANSFORMERS_MAPPING:
        if "llama" in _gguf_utils.GGUF_TO_TRANSFORMERS_MAPPING[section]:
            mapping = dict(_gguf_utils.GGUF_TO_TRANSFORMERS_MAPPING[section]["llama"])
            _gguf_utils.GGUF_TO_TRANSFORMERS_MAPPING[section]["olmo2"] = mapping
    if "llama" in GGUF_TO_FAST_CONVERTERS:
        GGUF_TO_FAST_CONVERTERS["olmo2"] = GGUF_TO_FAST_CONVERTERS["llama"]
...
class ModelVariant(StrEnum):
    """Available OLMo 3.1 32B Think GGUF model variants for causal language modeling."""

    OLMO_3_1_32B_THINK_Q4_K_M = "Q4_K_M"


class ModelLoader(ForgeModel):
    """OLMo 3.1 32B Think GGUF model loader implementation for causal language modeling tasks."""

    _VARIANTS = {
        ModelVariant.OLMO_3_1_32B_THINK_Q4_K_M: LLMModelConfig(
            pretrained_model_name="bartowski/allenai_Olmo-3.1-32B-Think-GGUF",
            max_length=256,
        ),
    }

    DEFAULT_VARIANT = ModelVariant.OLMO_3_1_32B_THINK_Q4_K_M

    GGUF_FILE = "allenai_Olmo-3.1-32B-Think-Q4_K_M.gguf"
