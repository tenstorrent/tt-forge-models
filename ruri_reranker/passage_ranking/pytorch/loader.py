...
class ModelVariant(StrEnum):
    """Available Ruri Reranker model variants for passage ranking."""
...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.SMALL: ModelConfig(
            pretrained_model_name="cl-nagoya/ruri-reranker-small",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.SMALL

    # Sample query-passage pairs for testing (Japanese, matching the model's language)
    sample_pairs = [
        (
            "瑠璃色はどんな色？",
            "瑠璃色は、紫みを帯びた濃い青色のことで、名前は仏教の七宝のひとつである瑠璃に由来しています。",
        ),
    ]
