...
# Map: mixer attribute base name → (gguf tensor base, gguf suffix override or None)
_MIXER_FALLBACK_MAP = {
    "in_proj": ("ssm_in", None),
...
def _get_kv_heads_from_reader(reader):
    """Read num_key_value_heads from per-layer GGUF field (take max non-zero value)."""
    try:
...
class _NemotronHMoeTensorProcessor:
    """Tensor processor for NemotronH MoE GGUF loading."""
...
def _apply_nemotron_h_moe_patch():
    """Monkey-patch transformers GGUF loading to support the nemotron_h_moe architecture."""
