...
# NOTE (tt-forge bringup): single-device vendored copy; context-parallel split
# is disabled (cp_split_hw defaults to [1, 1]) and its torch.distributed import
# is removed. The two split/gather branches below are never taken.
...
class LongCatVideoTransformer3DModel(
    ...
    def __init__(
        ...
        self.cp_split_hw = cp_split_hw if cp_split_hw is not None else [1, 1]
    ...
    def forward(
        ...
        with contextlib.nullcontext():
            t = self.t_embedder(timestep.float().flatten(), dtype=dtype).reshape(B, N_t, -1)  # [B, T, C_t] (tt-forge: model dtype)
        ...
        # context-parallel split removed (single-device); cp_split_hw == [1, 1]
        ...
        # context-parallel gather removed (single-device); cp_split_hw == [1, 1]
