...
# NOTE (tt-forge bringup): flash-attn / block-sparse-attention / ulysses
# context-parallel imports are dropped. A device-friendly SDPA fallback is used
# in the `else` branch of both attention helpers (all enable_* flags are False).
...
class Attention(nn.Module):
    ...
    def _process_attn(self, q, k, v, shape):
        ...
        if self.enable_bsa and shape[0] > 1: # bsa will not be used in image training / sampling
            ...
            raise RuntimeError("block-sparse attention is not available in the tt-forge vendored copy; keep enable_bsa=False")
            ...
            # device-friendly fallback: standard scaled-dot-product attention.
            # q, k, v: [B, H, S, D]
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def _process_cross_attn(self, x, cond, kv_seqlen):
        ...
        if self.enable_flashattn3:
            ...
            # device-friendly fallback: block-diagonal cross attention expressed
            # as a batched SDPA. Valid when every sample shares the same kv
            # length (the encoder_attention_mask=None path used for bringup).
            B_ = len(kv_seqlen)
            assert all(sl == kv_seqlen[0] for sl in kv_seqlen), (
                "SDPA cross-attn fallback requires uniform kv_seqlen; pass "
                "encoder_attention_mask=None so text tokens are not packed."
            )
            Nq = q.shape[1] // B_
            Sk = k.shape[1] // B_
            qh = q.view(B_, Nq, self.num_heads, self.head_dim).transpose(1, 2)
            kh = k.view(B_, Sk, self.num_heads, self.head_dim).transpose(1, 2)
            vh = v.view(B_, Sk, self.num_heads, self.head_dim).transpose(1, 2)
            xo = torch.nn.functional.scaled_dot_product_attention(
                qh, kh, vh, attn_mask=None, dropout_p=0.0
            )
            x = xo.transpose(1, 2).reshape(1, B_ * Nq, self.num_heads * self.head_dim)
