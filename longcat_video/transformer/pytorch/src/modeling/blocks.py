...
def modulate_fp32(norm_func, x, shift, scale):
    ...
    # tt-forge bringup: run the affine in fp32 internally (self-contained) and
    # cast back; do not require the modulation params to be fp32 so the model is
    # self-consistent in a single dtype on CPU and on device.
    dtype = x.dtype
...
class FinalLayer_FP32(nn.Module):
    ...
    def forward(self, x, t, latent_shape):
        # timestep shape: [B, T, C]  (tt-forge: dtype assert relaxed)
        B, N, C = x.shape
        T, _, _ = latent_shape
