...
class ModelLoader(ForgeModel):
    ...
    def load_shard_spec(self, model):
        ...
        (down) row-parallel (None, "model"). This alone drops per-chip DiT weights
        from ~27 GB to ~17 GB (fits a 32 GB Blackhole chip with activation
        headroom). The attention `qkv` is a single combined Linear(dim, 3*dim)
        whose (3, heads, head_dim) view does not align with a flat column split,
        so it (and `proj`, whose input would then be sharded) is kept replicated
        -- correct, just not memory-optimal. Splitting qkv into 3 head-parallel
        projections is the follow-up to shard attention too."""
