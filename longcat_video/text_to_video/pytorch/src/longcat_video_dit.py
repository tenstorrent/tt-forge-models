...
# (longcat_video/modules/longcat_video_dit.py). The context-parallel split is
# unused for single-device bringup (cp_split_hw == [1, 1]); its import is
# dropped. Attention is provided by the SDPA-patched local attention module.
...
class LongCatVideoTransformer3DModel(
    ...
    def forward(
        ...
        if self.text_tokens_zero_pad and encoder_attention_mask is not None:
            # Bringup note: with text_tokens_zero_pad the padded text tokens are
            # zeroed here and the attention mask is then reset to all-ones, so
            # the subsequent masked_select below keeps *every* token (it is an
            # identity flatten, never a true gather). The original data-dependent
            # masked_select + .tolist() produces dynamic shapes that break a
            # static-graph compiler, so we take the equivalent static path:
            # zero the padded embeddings and keep all M tokens as keys.
            encoder_hidden_states = encoder_hidden_states * encoder_attention_mask[:, None, :, None]
            B_y, _, M_tok, _ = encoder_hidden_states.shape
            y_seqlens = [M_tok] * B_y  # [B]
            encoder_hidden_states = encoder_hidden_states.squeeze(1).reshape(
