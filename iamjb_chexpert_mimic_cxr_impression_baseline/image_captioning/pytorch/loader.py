...
"""
IAMJB CheXpert MIMIC-CXR impression baseline model loader implementation (PyTorch).

A VisionEncoderDecoder model (ViT encoder + BERT decoder) that generates
radiology report impressions from chest X-ray images.
"""
...
class ModelVariant(StrEnum):
    ...
    IMPRESSION_BASELINE = "impression-baseline"
...
class ModelLoader(ForgeModel):
    ...
    _VARIANTS = {
        ModelVariant.IMPRESSION_BASELINE: ModelConfig(
            pretrained_model_name="IAMJB/chexpert-mimic-cxr-impression-baseline",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.IMPRESSION_BASELINE
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        decoder_input_ids = torch.tensor(
            [[self.tokenizer.cls_token_id]] * max(batch_size, 1)
        )
