...
Loads GGUF-quantized Wan 2.2 Fun InP transformers from
QuantStack/Wan2.2-Fun-A14B-InP-GGUF. The GGUF repo does not include a
config.json, so we use from_single_file with GGUFQuantizationConfig.
"""
import torch
...
class ModelLoader(ForgeModel):
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        encoder_hidden_states = torch.randn(
            batch_size, TRANSFORMER_TEXT_SEQ_LEN, config.text_dim, dtype=dtype
        )
