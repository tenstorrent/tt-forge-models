...
"""
Helper functions for Hyper-SD model loading and SDXL preprocessing.
"""
...
def load_pipe(base_model, lora_repo, lora_file):
    """Load SDXL pipeline with Hyper-SD LoRA weights applied.

    Args:
        base_model: HuggingFace model identifier for the SDXL base model.
        lora_repo: HuggingFace repo containing LoRA weights.
        lora_file: Filename of the LoRA safetensors within lora_repo.

    Returns:
        DiffusionPipeline: Loaded pipeline with LoRA fused and DDIM scheduler.
    """
    pipe = DiffusionPipeline.from_pretrained(
        base_model, torch_dtype=torch.float32, variant="fp16"
    )

    pipe.load_lora_weights(hf_hub_download(lora_repo, lora_file))
    pipe.fuse_lora()
...
def stable_diffusion_preprocessing_xl(
    ...
    Args:
        pipe: SDXL DiffusionPipeline instance.
