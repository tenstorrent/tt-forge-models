...
class ModelLoader(ForgeModel):
    ...
    @classmethod
    def decode_output(
        cls, outputs=None, inputs=None, tokenizer: Optional[BertTokenizer] = None
    ):
        """Decode the model output for masked language modeling."""
        if outputs is None or inputs is None or tokenizer is None:
            return None
        logits = outputs[0]
        input_ids = inputs[0]
        mask_idx = (
            (input_ids == tokenizer.mask_token_id)[0].nonzero(as_tuple=True)[0].item()
        )
        predicted_token_id = logits[0, mask_idx].argmax(axis=-1).item()
