...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, **kwargs):
        ...
        # The tt_forge_models/llm2vec/ model directory can be picked up as a
        # namespace package, causing gliner's ``is_module_available("llm2vec")``
        # check to return True even though the real llm2vec PyPI package is not
        # installed.  Remove the bogus entry so the optional-import guard in
        # gliner.modeling.encoder correctly falls back to the no-llm2vec path.
        _mod = sys.modules.get("llm2vec")
        if _mod is not None and not hasattr(_mod, "__file__"):
            del sys.modules["llm2vec"]
        elif _mod is None:
            try:
                _spec = importlib.util.find_spec("llm2vec")
                if _spec is not None and _spec.origin is None:
                    sys.modules["llm2vec"] = None  # type: ignore[assignment]
