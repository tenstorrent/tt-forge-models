...
def _register_qwen3vl_gguf_architecture():
    ...
    The GGUF stores general.architecture = "qwen3vl" (no underscore).
    transformers does not have qwen3vl in GGUF_SUPPORTED_ARCHITECTURES, so
    load_gguf_checkpoint raises ValueError. Also, get_gguf_hf_weights_map uses
    hf_model.config.model_type = "qwen3_vl" (with underscore), which gguf-py
    cannot find in MODEL_ARCH_NAMES since it expects "qwen3vl".
...
class ModelLoader(ForgeModel):
    ...
    # Base model provides processor and correct Qwen3VLConfig for from_pretrained
