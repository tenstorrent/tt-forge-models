...
def load_qwen_image_edit_plus_pipeline(
    base_model, lora_repo, lora_weight_name, dtype=torch.bfloat16
):
    pipe = QwenImageEditPlusPipeline.from_pretrained(base_model, torch_dtype=dtype)
...
def qwen_image_edit_plus_preprocessing(pipe, prompt, images, device="cpu"):
    ...
    for img in images:
        ...
        condition_images.append(
            pipe.image_processor.resize(img, condition_height, condition_width)
        )
        vae_images.append(
            pipe.image_processor.preprocess(img, vae_height, vae_width).unsqueeze(2)
        )
