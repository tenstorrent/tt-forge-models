...
import transformers.models.mt5.modeling_mt5 as _mt5
...
_HEAD_MASK_WARNING_MSG = getattr(
    ...
    "The input argument `head_mask` was split into two arguments.",
)


@dataclasses.dataclass
class _MT5ForRegressionOutput(transformers.modeling_outputs.ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    predictions: torch.FloatTensor = None


class _MT5ForRegression(_mt5.MT5PreTrainedModel):
    """MT5 model for regression, adapted from google-research/metricx."""

    def __init__(self, config: _mt5.MT5Config):
        ...
        self.encoder = _mt5.MT5Stack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = _mt5.MT5Stack(decoder_config, self.shared)
    ...
    def forward(
        ...
        predictions = torch.clamp(lm_logits[:, 0, 250089], 0, 25)
        ...
        return _MT5ForRegressionOutput(loss=loss, predictions=predictions)
...
class ModelLoader(ForgeModel):
    ...
    def load_model(self, *, dtype_override=None, **kwargs):
        ...
        model = _MT5ForRegression.from_pretrained(pretrained_model_name, **model_kwargs)
        model.eval()
        return model
