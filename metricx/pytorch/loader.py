...
MT5ForRegression is inlined from https://github.com/google-research/metricx
(metricx24/models.py) since that repo has no installable package.
...
_BaseModelOutput = transformers.modeling_outputs.BaseModelOutput
_ModelOutput = transformers.modeling_outputs.ModelOutput
_MT5Config = transformers.models.mt5.modeling_mt5.MT5Config
_MT5PreTrainedModel = transformers.models.mt5.modeling_mt5.MT5PreTrainedModel
_MT5Stack = transformers.models.mt5.modeling_mt5.MT5Stack
_HEAD_MASK_WARNING_MSG = getattr(
    transformers.models.mt5.modeling_mt5,
    "__HEAD_MASK_WARNING_MSG",
    "head_mask is deprecated, use decoder_head_mask instead",
)


@dataclasses.dataclass
class MT5ForRegressionOutput(_ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    predictions: torch.FloatTensor = None


class MT5ForRegression(_MT5PreTrainedModel):
    """MT5 model for regression (inlined from google-research/metricx)."""

    def __init__(self, config: _MT5Config):
        ...
        self.encoder = _MT5Stack(encoder_config)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = _MT5Stack(decoder_config)
    ...
    def forward(
        ...
        elif return_dict and not isinstance(encoder_outputs, _BaseModelOutput):
            encoder_outputs = _BaseModelOutput(
