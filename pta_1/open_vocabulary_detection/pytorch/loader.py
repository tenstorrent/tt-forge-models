...
"""
PTA-1 (Prompt-to-Automation) open vocabulary detection model loader (PyTorch).
"""
...
class ModelVariant(StrEnum):
    """Available PTA-1 model variants for open vocabulary detection."""

    BASE = "Base"


class ModelLoader(ForgeModel):
    """PTA-1 model loader implementation for open vocabulary detection of GUI elements."""

    _VARIANTS = {
        ModelVariant.BASE: ModelConfig(
            pretrained_model_name="AskUI/PTA-1",
        ),
    }
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        ...
        prompt = "<OPEN_VOCABULARY_DETECTION>cat"
        inputs = self.processor(text=prompt, images=image, return_tensors="pt")

        # PTA-1 is a Florence-2 based seq2seq model that requires decoder_input_ids
