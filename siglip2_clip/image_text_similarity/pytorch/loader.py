...
"""
SigLIP2 CLIP model loader implementation for image-text similarity using OpenCLIP.
"""
import torch
...
class ModelVariant(StrEnum):
    """Available SigLIP2 CLIP model variants."""
...
class ModelLoader(ForgeModel):
    """SigLIP2 CLIP model loader using OpenCLIP for image-text similarity tasks."""

    _VARIANTS = {
        ModelVariant.VIT_SO400M_16_384: ModelConfig(
            pretrained_model_name="hf-hub:timm/ViT-SO400M-16-SigLIP2-384",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.VIT_SO400M_16_384
    ...
    def load_inputs(self, dtype_override=None, batch_size=1):
        """Load and return sample inputs for the SigLIP2 CLIP model.
        ...
        # Tokenize text - call underlying HF tokenizer directly since
        # open_clip's HFTokenizer uses batch_encode_plus removed in transformers 5.x
        hf_tok = self.tokenizer.tokenizer
        ctx_len = self.tokenizer.context_length
        text_tokens = hf_tok(
            self.text_prompts,
            return_tensors="pt",
            max_length=ctx_len,
    ...
    def post_process(self, outputs):
        """Post-process SigLIP2 CLIP model outputs to extract similarity scores.
