...
def build_transform(input_size):
    """Build image transform pipeline for Sa2VA."""
    return T.Compose(
        [
...
def load_image(image_path, input_size=448, max_num=12):
    """Load and preprocess an image for Sa2VA."""
...
class ModelLoader(ForgeModel):
    """Sa2VA model loader for multimodal visual question answering."""

    _VARIANTS = {
        ModelVariant.SA2VA_4B: ModelConfig(
            pretrained_model_name="ByteDance/Sa2VA-4B",
        ),
    }

    DEFAULT_VARIANT = ModelVariant.SA2VA_4B
